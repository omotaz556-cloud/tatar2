...
return false;}
function we(){if(fe){pb=parent.document.getElementById("drag2");if(pb!=null){pb.innerHTML=''}else{window.close();}}
...
function vf(qf,rf,wf){
	if(wf==null){wf=0;}
	if(m_c.size==null){
		throw'Globale Variable m_c.size muss auf den Wert von $novaterra[map_prefetch_rows]) gesetzt werden.';
	}
	var xf,yf;
	if(null===rf||1===rf){
		yf=m_c.size-1;
