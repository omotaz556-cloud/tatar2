...
#################################################################################
		...
		if (
		    ...
		    // اللغة المختارة وقت التثبيت (config.tpl -> name="lang"). لو جاءت فاضية لأي سبب،
		    // نرجع لـ LANG (الثابت العام اللي اتكتب في config.php) بدل ما نسيبها تاخد
		    // الديفولت 'en' من قاعدة البيانات.
		    $adminLang = !empty($_POST['lang']) ? $_POST['lang'] : LANG;

		    mysqli_query($database->dblink, "INSERT INTO " . TB_PREFIX . "users SET username = '".$database->escape($_POST['aname'])."', password = '" . password_hash($_POST['apass'], PASSWORD_BCRYPT, ['cost' => 12]) . "', email = '".$database->escape($_POST['aemail'])."', tribe = ".(int) $_POST['atribe'].", access = 9, is_bcrypt = 1, lang = '".$database->escape($adminLang)."', desc1 = '[#MH]\n[#TEAM]', desc2 = '[#MULTIHUNTER]\n[#roman]'") OR DIE (mysqli_error($database->dblink));
