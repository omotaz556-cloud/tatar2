# Novaterra Docker Setup

This guide will help you set up Novaterra using Docker and Docker Compose for easy deployment.
...
MARIADB_DATABASE=novaterra
MARIADB_USER=novaterra
...
This command will:
- Build the Novaterra web application container
...
The installation wizard creates the Novaterra schema. SQL files in `var/db`
...
- **Port:** `3306`
- **Username:** `novaterra` (or the value from your `.env` file)
- **Password:** `novaterrapass` (or the value from your `.env` file)
- **DB name:** `novaterra` (or the value from your `.env` file)
...
- **Novaterra Web Application:** http://localhost:8080
...
```bash
docker exec -it novaterra-web bash
```

### Access MySQL Container

```bash
docker exec -it novaterra-db mysql -u root -p
```
...
```bash
docker exec -it novaterra-web chown -R www-data:www-data /var/www/html
docker exec -it novaterra-web chmod -R 777 /var/www/html/var
```
...
```bash
docker exec novaterra-db mariadb-dump -u root -p novaterra > backup_$(date +%Y%m%d).sql
```

### Restore Database

```bash
docker exec -i novaterra-db mariadb -u root -p novaterra < backup_20231125.sql
```

### Backup Application Files

```bash
tar -czf novaterra_backup_$(date +%Y%m%d).tar.gz \
  --exclude='./var/db' \
  --exclude='./.git' \
  .
```
...
To update Novaterra to the latest version:
...
Novaterra Project - See LICENSE file for details
