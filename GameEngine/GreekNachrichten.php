...
class GreekNachrichten
{
    ...
    public static function menuOpen($tab)
    {
        ...
        // Use & not &amp; — htmlspecialchars() encodes & for HTML attributes.
        $chatHref = 'allianz.php?s=6&public=1';
        ...
        $tabs = array(
            ...
            array('href' => $chatHref, 'label' => defined('TZ_MSG_CHAT_TAB') ? TZ_MSG_CHAT_TAB : 'الشات', 't' => -1),
        );

        if (!empty($session->plus)) {
            $tabs[] = array('href' => 'nachrichten.php?t=4', 'label' => defined('TZ_MSG_NOTES_TAB') ? TZ_MSG_NOTES_TAB : 'دفتر الملاحظات', 't' => 4);
        }

        $tabs[] = array('href' => 'nachrichten.php?t=5', 'label' => defined('TZ_MSG_IGNORED_TAB') ? TZ_MSG_IGNORED_TAB : 'المتجاهلون', 't' => 5);
