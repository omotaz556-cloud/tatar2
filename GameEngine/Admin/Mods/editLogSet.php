...
##  Copyright      : Novaterra mods (c) 2010-2014; base engine (c) TravianZ authors (GPLv3). ##
##                                                                             ##
#################################################################################
...
if (!admin_config_template_available()) {
    die(
        'You seem to be running a new version of Novaterra which was installed using an old installer.<br />' .
        ...
        '<strong>https://raw.githubusercontent.com/omotaz556-cloud/tatar/main/install/data/constant_format.tpl</strong>');
}
...
$myFile = "../../config.php";
		...
		tz_config_set($text, '%ERRORREPORT%', $ERRORREPORT);
		tz_config_set($text, '%ERROR%', $ERRORREPORT, 'code');
		...
		tz_config_set($text, '%WW%', $WW);
		tz_config_set($text, '%SHOW_NATARS%', $SHOW_NATARS);
		...
		tz_config_set($text, '%ACTIVATE%', $AUTH_EMAIL);
		...
		tz_config_set($text, '%GREAT_WKS%', $GREAT_WKS);
		tz_config_set($text, '%TS_THRESHOLD%', TS_THRESHOLD);
		tz_config_set($text, '%REG_OPEN%', $REG_OPEN);
		tz_config_set($text, '%PEACE%', PEACE);
		//update
		tz_config_set($text, '%LOGBUILD%', $_POST['log_build'] ?? '');
		tz_config_set($text, '%LOGTECH%', $_POST['log_tech'] ?? '');
		tz_config_set($text, '%LOGLOGIN%', $_POST['log_login'] ?? '');
		tz_config_set($text, '%LOGGOLDFIN%', $_POST['log_gold_fin'] ?? '');
		tz_config_set($text, '%LOGADMIN%', $_POST['log_admin'] ?? '');
		tz_config_set($text, '%LOGWAR%', $_POST['log_war'] ?? '');
		tz_config_set($text, '%LOGMARKET%', $_POST['log_market'] ?? '');
		tz_config_set($text, '%LOGILLEGAL%', $_POST['log_illegal'] ?? '');
		//end update
