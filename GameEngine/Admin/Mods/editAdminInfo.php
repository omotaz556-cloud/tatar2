...
$myFile = "../../config.php";
		...
		tz_config_set($text, '%ARANK%', $_POST['admin_rank'] ?? '');
		tz_config_set($text, '%AEMAIL%', $_POST['aemail'] ?? '');
		tz_config_set($text, '%ANAME%', $_POST['aname'] ?? '');
		tz_config_set($text, '%ASUPPMSGS%', ($_POST['admin_support_msgs'] == 'True' ? 'true' : 'false'));
		tz_config_set($text, '%ARAIDS%', ($_POST['admin_raidable'] == 'True' ? 'true' : 'false'));
