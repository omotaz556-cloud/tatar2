 ...
 *
 * Un director din gpack/ conteaza ca pachet valid daca are novaterra.css - acelasi
...
if (!function_exists('tz_available_gpacks')) {
    function tz_available_gpacks($root = null)
    {
        ...
        foreach (scandir($root) as $dir) {
            ...
            if (!is_dir($root . '/' . $dir) || !is_file($root . '/' . $dir . '/novaterra.css')) {
                continue;
            }
...
if (!function_exists('tz_config_finalize')) {
    ...
    function tz_config_finalize($text)
    {
        ...
        foreach ($leftovers as $row) {
            ...
            error_log('[Novaterra] config: placeholder ' . $ph[0]
