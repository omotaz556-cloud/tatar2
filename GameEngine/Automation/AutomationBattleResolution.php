...
## --------------------------------------------------------------------------- ##
##  Project:       Novaterra                                                    ##
##  Filename:      AutomationBattleResolution.php                              ##
##  Split&Refactor Shadow													   ##
##  Purpose:       sendunitsComplete + all battle resolution helper methods    ##
##                                                                             ##
...
trait AutomationBattleResolution {
	...
	*/
     ...
     * @param int   $attackerUid Attacker's uid (from['owner']) - used only for the FeedingSystem linked-account check.
     * @return array ['totclay','totiron','totwood','totcrop','avclay','aviron','avwood','avcrop'].
     */
    private function resolveResourcesAfterBattle($data, $isoasis, $conqureby, $owntribe, $targettribe, &$crannySpy, $attackerUid = 0) {
        ...
        if ($isoasis == 0){
            ...
            // the attacker has declared this village's owner as one of
    ...
    private function sendunitsComplete() {
		// COMPLETE ATTACK PROCESSING - critical function, kept 100% compatible
		// This function handles all attacks that reach their destination.
		// Includes: battles, traps, hero evasion, building destruction, and village conquest.
        ...
        if ($dataarray && count($dataarray)) {
            ...
            foreach($dataarray as $data) {
                ...
                if(PEACE == 0 || $targettribe == 4 || $targettribe == 5 || $scout){
                    ...
                    // ($from['owner'] passed through only for the FeedingSystem linked-account check - see docblock)
                    $resAfter = $this->resolveResourcesAfterBattle($data, $isoasis, $conqureby, $owntribe, $targettribe, $crannySpy, (int) $from['owner']);
