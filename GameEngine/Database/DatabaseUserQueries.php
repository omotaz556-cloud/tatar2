...
trait DatabaseUserQueries {
    ...
    if($stmt->execute()){
        ...
        $this->syncCentralGoldOnRegister($id, $username, $email);
        return $id;
    }
    $stmt->close();
	...
	/**
	 * Central-gold sync hook, called once right after a new account is
	 * created. If the deployment has CENTRAL_GOLD_* configured (see
	 * config.php / GameEngine/CentralGold.php) and this player already has a
	 * portable paid-gold balance from another world (same email), that
	 * balance is pulled into this world's local `users.gold` so it "just
	 * shows up" the moment they log in — no separate claim step needed.
	 *
	 * No-ops completely (including no extra DB connection attempt) when
	 * central gold isn't configured for this deployment, so single-world
	 * installs are entirely unaffected. See GameEngine/CentralGold.php for
	 * the full explanation of why this is a "sync at known points" design
	 * rather than a rewrite of every local gold-spending code path.
	 */
	private function syncCentralGoldOnRegister($id, $username, $email) {
		$id = (int) $id;
		if ($id <= 3 || $email === '') {
			return; // system accounts, or no email to key off of
		}
