...
//COPYRIGHT
tz_def('NOVATERRA_COPYRIGHT', 'Novaterra 100% Open Source Novaterra Clone.');
...
tz_def('NATAR_COUNTERFORCE', 'Tatar Counterforce');
...
tz_def('TZ_WARSIM_TYPE_OF_ATTACK', 'Type of attack');
tz_def('TZ_WARSIM_NORMAL', 'Normal');
tz_def('TZ_WARSIM_RAM_HINT', 'Hint: The ram does not work during a raid.');
tz_def('TZ_WARSIM_CATA_HINT', 'Hint: The catapult does not shoot during a raid.');
tz_def('TZ_WARSIM_RAM_DAMAGE', 'Damage done by ram: from level <b>%s</b> to level <b>%s</b>');
tz_def('TZ_WARSIM_CATA_DAMAGE', 'Damage done by catapult: from level <b>%s</b> to level <b>%s</b>');
...
tz_def('Q2_REWARD', 'Your reward:</p>1 day Novaterra');
...
tz_def('Q9_DESC', 'In Novaterra there is always something to do! While you are waiting for incoming the huge army, Now we should increase your resource production a bit. Extend all your resource tiles to level 1.');
...
tz_def('Q10_REWARD', 'Your reward:</p>2 day Novaterra');
...
tz_def('Q28_DESC', 'Teamwork is important in Novaterra. Players who work together organise themselves in alliances. Get an invitation from an alliance in your region and join this alliance. Alternatively, you can found your own alliance. To do this, you need a level 3 embassy.');
...
tz_def('ONE_DAY_OF_NOVATERRA', '1 day Novaterra ');
tz_def('TWO_DAYS_OF_NOVATERRA', '2 days Novaterra ');
...
tz_def('Q25_10_REWARD', 'Your reward:</p>2 days more of Novaterra');
...
tz_def('Q25_28_DESC', 'The world of Novaterra is dangerous. You already built a cranny to protect you against attackers.<br><br>A good alliance will give you even better protection.');
...
tz_def('Q25_29_DESC', 'Teamwork is important in Novaterra. Players who work together organise themselves in alliances. Get an invitation from an alliance in your region and join this alliance. Alternatively, you can found your own alliance. To do this, you need a level 3 embassy.');
...
tz_def('U45', 'Axerider');
tz_def('U46', 'Tatarian Knight');
tz_def('U47', 'War Elephant');
tz_def('U48', 'Ballista');
tz_def('U49', 'Tatarian Emperor');
tz_def('U50', 'Tatarian Settler');
//HUNS (TRIBE 6)
...
tz_def('BEFORE_REGISTER', 'Before you register an account you should read the <a href="/anleitung.php" target="_blank">instructions</a> of Novaterra ro1 to see the specific advantages and disadvantages of the each tribes.');
...
tz_def('TZ_UPGRADE_NAVIGATION', 'After-upgrade view');
tz_def('TZ_UPGRADE_NAVIGATION_DEFAULT', 'Default: level 1 on the map, then stay in the same building');
tz_def('TZ_UPGRADE_NAVIGATION_MAP', 'Always open the village map');
tz_def('TZ_UPGRADE_NAVIGATION_BUILDING', 'Always stay in the same building');
...
tz_def('HERO_TYPE', 'Hero type');
tz_def('HERO_ACTIVITY', 'Action');
tz_def('HERO_NEW_STARTS', 'New hero starts at level 1');
tz_def('HERO_INFO_TITLE', 'Hero information');
tz_def('HERO_LABEL_NAME', 'Name');
tz_def('HERO_LABEL_HEALTH', 'Health');
tz_def('HERO_LABEL_TYPE', 'Type');
tz_def('HERO_LABEL_OFF_ARMY', 'Army attack under hero');
tz_def('HERO_LABEL_DEF_ARMY', 'Army defence under hero');
tz_def('HERO_LABEL_INDIVIDUAL', 'Hero individual strength');
tz_def('HERO_LABEL_REGEN', 'Regeneration');
tz_def('HERO_LABEL_STATUS', 'Hero status');
tz_def('HERO_LABEL_CARRY', 'Carry when raiding');
tz_def('HERO_LABEL_POINTS_NOW', 'Hero points now');
tz_def('HERO_LABEL_POINTS_NEXT', 'Hero points for level %d');
tz_def('HERO_STATUS_READY', 'Ready to defend the village');
tz_def('HERO_STATUS_AWAY', 'Hero is away');
tz_def('HERO_STATUS_DEAD', 'Hero is dead');
tz_def('HERO_RES_UNIT', 'resources');
tz_def('HERO_POINT_UNIT', 'points');
tz_def('NO_HERO_UNITS', 'No soldiers available to train as a hero. Train a soldier first.');
...
tz_def('HEROSMANSION_DESC', 'In the hero mansion you can conquer nearby oases; the higher the mansion level, the more oases you can occupy. You can also revive your hero with his experience or train a new hero.');
tz_def('OASIS_OWNED_OF', 'You have conquered %d of %d oases; upgrade the building for more');
tz_def('OASIS_RANGE_HINT', 'Oases outside the 7 x 7 square cannot be occupied; increasing population to %s expands the square');
tz_def('OASIS_VIEW_NEARBY', 'Show nearby oases you can occupy');
tz_def('OASIS_EXPAND_POP', '1020');
tz_def('OASIS_MAX_NOW', 'Current maximum');
tz_def('OASIS_MAX_NEXT', 'Maximum at level');
tz_def('OASIS_UNIT_SING', 'oasis');
tz_def('OASIS_IN_SQUARE', 'In square');

tz_def('GREATWAREHOUSE', 'Great Warehouse');
tz_def('GREATWAREHOUSE_DESC', 'The great warehouse has 3 times the capacity of a normal warehouse.<br>This building can only be built in wonder of the world villages or with a special Tatarian artefact');

tz_def('GREATGRANARY', 'Great Granary');
tz_def('GREATGRANARY_DESC', 'The great granary has 3 times the capacity of a normal granary.<br>This building can only be built in wonder of the world villages or with a special Tatarian artefact');

tz_def('WONDER', 'Wonder of the World');
tz_def('WORLD_WONDER', 'World Wonder');
tz_def('WONDER_DESC', 'A Wonder of the World (also known as WW) is as astonishing as it sounds. Every level costs a lot of resources. It is nearly impossible for a single player to build a WW on their own. The reason is that you not only need a lot of resources, but also the troops to protect your precious building.<br>To build WW you need an Ancient Construction Plan. You can get this by attacking a Tatar Village with your hero. You need to have an empty level 10 treasury and your hero must survive. With those plan and an extremely high level of resources, you can start the world wonder.<br>Once it gets to level 50, you will need someone else in your alliance to have a second construction plan active. You cannot do it all by yourself.<br>Finishing a WW lvl 100, you will win the Novaterra server and its the end of a game world.<br>Once you finish, a message will come up telling who won and statistics. You can no longer build, but you can message people until the server restarts');
...
tz_def('DELETE_ACCOUNT2', 'You can delete your account here. Once you start deleting your account, it remains active for one day. You can cancel the deletion within the first 24 hours.');
tz_def('PREF_VACATION_HUB_HINT', 'Vacation fully disables the account for three days (it ends automatically when the period finishes, or manually anytime if you remove it yourself). Activating it costs 5000 gold, and canceling early also costs 5000 gold. You can activate vacation again one week after the first one ends.');
tz_def('PREF_VACATION_TITLE', 'Vacation');
tz_def('PREF_WEB_NOTIF_BODY', 'Web notifications are enabled in this browser.');
tz_def('PREF_WEB_NOTIF_TITLE', 'Web notifications');
tz_def('YES', 'Yes');
tz_def('NO', 'No');
...
tz_def('NATARS_ACC', 'Official Tatar account'); // natars
...
tz_def('NATURE_M2', 'Natures troops are the animals living in unoccupied oases. You can use the combat simulator to see whether you have enough troops to defeat the animals in an oasis you want to conquer, but remember that you can only raid oasis. Keep in mind that all the animals above Bear can kill its contemporary max tier novaterra troop in single combat');
tz_def('TASKMASTER_M', 'Taskmaster Account');
tz_def('VETERAN_P', 'Veteran Player');
tz_def('VETERAN_3_M', 'Medal achieved for playing 3 years of Novaterra');
tz_def('VETERAN_5_M', 'Medal achieved for playing 5 years of Novaterra');
tz_def('VETERAN_10_M', 'Medal achieved for playing 10 years of Novaterra');
...
tz_def('PREF_MOBILE_MODE', 'Mobile mode');
tz_def('PREF_OPTIONS', 'Options');
tz_def('PREF_WEB_NOTIFICATIONS', 'Web notifications');
tz_def('PREF_LINKED_ACCOUNTS', 'Linked accounts');
tz_def('PREF_MANAGE', 'Manage');
tz_def('PREF_OPEN_OPTIONS', 'Open display options');
tz_def('PREF_NOTIFICATION_SETTINGS', 'Notification settings');
tz_def('PREF_LINKED_ACCOUNTS_SETTINGS', 'Linked account settings');
tz_def('PREF_MOBILE_AUTO', 'Automatic');
tz_def('PREF_MOBILE_DESKTOP', 'Computer');
tz_def('PREF_MOBILE_PHONE', 'Mobile phone');
tz_def('PREF_TIMER_REFRESH', 'Refresh when timer ends');
tz_def('PREF_INVERT_COLORS', 'Invert colors');
tz_def('PREF_STATS_FORMAT', 'Statistics format');
tz_def('PREF_STATS_AUTO', 'Automatic');
tz_def('PREF_STATS_CLASSIC', 'Classic');
tz_def('PREF_STATS_COMPACT', 'Compact');
tz_def('PREF_NIGHT_MODE', 'Night mode');
tz_def('PREF_NIGHT_AUTO', 'Automatic');
tz_def('PREF_LIGHT_MODE', 'Light mode');
tz_def('PREF_DARK_MODE', 'Dark mode');
tz_def('PREF_NAME_RESERVATION', 'Name reservation');
tz_def('PREF_HERO_NAME_MANAGE', 'Manage hero name');
tz_def('PREF_HERO_COUNT', 'Registered heroes');
tz_def('PREF_YES', 'Yes');
tz_def('PREF_NO', 'No');
tz_def('PREF_SECTION_PREMIUM', 'Premium services');
tz_def('PREF_SECTION_AUTH', 'Authorization');
tz_def('PREF_SECTION_OPTIONS', 'Options');
tz_def('PREF_SECTION_NOTIF', 'Notifications');
tz_def('PREF_CHANGE_NAME', 'Change name');
tz_def('PREF_CHANGE_NAME_HINT', 'Change hero or village name');
tz_def('PREF_SITTER_NOTIF', 'Sitter notifications');
tz_def('PREF_SITTER_NOTIF_HINT', 'Alerts when a sitter logs into your account');
tz_def('PREF_STOP_AUTO_UPDATE', 'Stop auto-refresh');
tz_def('PREF_BLOCK_MESSAGES', 'Block sending messages');
tz_def('PREF_REDIRECT_BEHAVIOR', 'Redirect after upgrade');
tz_def('PREF_MAP_STYLE', 'Map style');
tz_def('PREF_MAP_STYLE_HINT', 'Large map or normal');
...
tz_def('TIME_ZONES_DESC', 'Here you can change Novaterra`s displayed time to fit your time zone');
...
tz_def('ARTEFACT', '<h1><b>Tatars Artefacts</b></h1>
...
Countless days have passed since the first battles upon the walls of the cursed villages of the Dread Tatars, many armies of both the free ones and the Tatarian empire struggled and died before the walls of the many strongholds from which the Tatars had once ruled all creation. Now with the dust settled and a relative calm having settled in, armies began to count their losses and collect their dead, the stench of combat still lingering in the night air, a smell of a slaughter unforgettable in its extent and brutality yet soon to be dwarfed by yet others. The largest armies of the free ones and the Dread Tatars were marshalling for yet another renewed assault upon the coveted former strongholds of the Tatarian Empire.
Soon scouts arrived telling of a most awesome sight and a chilling reminder, a dread army of an unfathomable size had been spotted marshalling at the end of the world, the Tatarian capital, a force so great and unstoppable that the dust from their march would choke off all light, a force so brutal and ruthless that it would crush all hope. The free people knew that they had to race now, race against time and the endless hordes of the Tatarian Empire to raise a Wonder of the World to restore the world to peace and vanquish the Tatarian threat.
...
Many moons ago the tribes of Novaterra were surprised by the unforeseen return of the Tatars. This tribe from immemorial times surpassing all in wisdom, might and glory was about to trouble the free ones again. Thus they put all their efforts in preparing a last war against the Tatars and vanquishing them forever. Many thought about the so-called "Wonders of the World", a construction of many legends, as the only solution. It was told that it would render anyone invincible once completed. Ultimately making the constructors the rulers and conquerors of all known Novaterra. 
...
Today, however, this last secret will be revealed. Deprivations and endeavors of the past will not have been in vain, as today scouts of several tribes have successfully obtained the whereabouts of the construction plans. Well guarded by the Tatars, they lie hidden in several oases to be found all over Novaterra. Only the most valiant heroes will be able to secure such a plan and bring it home safely so that the construction can begin. 
...
tz_def('CONF_SERV_NATUNITSMULTIPLIER', 'Tatars Units Multiplier');
tz_def('CONF_SERV_NATUNITSMULTIPLIER_TOOLTIP', 'This parameter is responsible for the number of troops of Tatars, on artefacts and WW villages.');
tz_def('CONF_SERV_NATARS_SPAWN_TIME', 'Tatars Spawn');
tz_def('CONF_SERV_NATARS_SPAWN_TIME_TOOLTIP', 'After how long Tatars and artefacts will spawn from the start date of the server, in days');
...
tz_def('CONF_SERV_NATARSTAT', 'Show Tatars in Statistics');
tz_def('CONF_SERV_NATARSTAT_TOOLTIP', 'Enable (True) or disable (False) the display of the Tatars account in statistics.');
...
$lang['index'][0][13] = 'Play with and against thousands of other real players and conquer the the world of Novaterra.';
...
tz_def('TZ_MSG_INBOX_TAB', 'Inbox');
tz_def('TZ_DAY_TODAY', 'Today');
tz_def('TZ_DAY_YESTERDAY', 'Yesterday');
tz_def('TZ_MSG_SENT_TAB', 'Sent');
tz_def('TZ_MSG_WRITE_TAB', 'Write');
tz_def('TZ_MSG_CHAT_TAB', 'Chat');
tz_def('TZ_MSG_NOTES_TAB', 'Notebook');
tz_def('TZ_MSG_IGNORED_TAB', 'Ignored');
tz_def('TZ_MSG_INBOX_TITLE', 'Inbox');
tz_def('TZ_MSG_SENT_TITLE', 'Sent messages');
tz_def('TZ_MSG_WRITE_TITLE', 'Write message');
tz_def('TZ_MSG_NOTES_TITLE', 'Notebook');
tz_def('TZ_MSG_ARCHIVE_TITLE', 'Archive');
...
tz_def('TZ_PROF_TAB_OPTIONS', 'Options');
tz_def('TZ_PROF_TAB_ACCOUNTS', 'Accounts');
tz_def('TZ_PROF_TAB_NAME_RESERVE', 'Name reservation');
tz_def('TZ_PROF_EDIT_INFO', 'Edit your personal information');
tz_def('TZ_PROF_RESOURCES_COL', 'Resources');
tz_def('TZ_PROF_TROOPS_COL', 'Troops');
tz_def('TZ_PROF_DISTANCE_COL', 'Distance');
tz_def('TZ_RPT_ATTACK_TAB', 'Attack');
...
tz_def('TZ_FEATURES_OF_NOVATERRA', 'Features of Novaterra');
...
tz_def('TZ_VACATION_REAL_GOLD_REQUIRED', 'Vacation Mode requires a real, purchased Gold balance. Gold granted by an administrator cannot be used to activate Vacation Mode.');
tz_def('ADM_VACATION_TEST_MODE', 'Vacation Test Mode (Admin Gold)');
tz_def('ADM_VACATION_TEST_MODE_DESC', 'TESTING ONLY. When ON, gold added to a player by an admin (Add Gold) is also accepted to activate Vacation Mode, so testers do not need a real purchase. When OFF (recommended for production), Vacation Mode only accepts a real, purchased Gold balance. This does not change how Paid/Free Gold is classified anywhere else in the game.');
tz_def('ADM_TESTING_ONLY_BADGE', 'TESTING ONLY');
tz_def('ADM_NOT_PRESENT_IN_CONFIG_PHP_YET_SAVING_THIS_FO', 'Not present in config.php yet — saving this form will add it.');
...
tz_def('TZ_RELEASED_BY_NOVATERRA_TEAM', 'Released by: Novaterra Team');
tz_def('TZ_RELEASE_BY_NOVATERRA', '[Release by: Novaterra]');
...
tz_def('TZ_ADDITIONALLY_THE_NOVATERRA_TEAM_WILL', 'Additionally, the Novaterra Team will not provide information concerning bans to any person, other than the account owner.');
tz_def('TZ_ADVERTISEMENT_OF_ANY_KIND_THAT_HAS', 'Advertisement of any kind that has not been permitted by the Novaterra Team is impermissible.');
...
tz_def('TZ_ANY_SALES_OR_PURCHASES_CONCERNING', 'Any sales or purchases concerning real money regarding accounts, units, villages, resources, services or any other aspect of Novaterra are impermissible. The sale of Novaterra accounts as well as any indirect transfer (even as gifts) in connection with auction sites or other money transactions is impermissible.');
...
tz_def('TZ_BLACKMAILING_PLAYERS_IN_A_WAY_THAT', 'Blackmailing players in a way that violates any of Novaterra\'s rules in accordance with the General Terms and Conditions.');
...
tz_def('TZ_HERE_YOU_CAN_CHANGE_NOVATERRA_S_DISP', 'Here you can change Novaterra\'s displayed time to fit your time zone.');
...
tz_def('TZ_INCITING_MANIPULATING_ENCOURAGING', 'Inciting, manipulating, encouraging, assisting or conspiring with others to violate any of Novaterra Rules is impermissible. These rules apply for players that will delete their accounts or are currently deleting their accounts without exception.');
...
tz_def('TZ_IN_ORDER_TO_PLAY_NOVATERRA_YOU_NEED', 'In order to play Novaterra you need a valid email address to which the activation code can be send. There are exceptional cases when this email might not arrive.');
...
tz_def('TZ_IN_ORDER_TO_SWITCH_AN_ACCOUNT_WITH', 'In order to switch an account with another person on the same game world, both persons must send an e-mail message to admin@novaterra.com from the e-mail address currently registered to the account. The e-mail must contain the following information:');
tz_def('TZ_IN_THE_BEGINNING_YOUR_SMALL_VILLAG', 'In the beginning your small village will have just one building.');
tz_def('TZ_IN_NOVATERRA_YOU_ARE_NOT_ALONE_YOU_I', 'In Novaterra you are not alone; you interact with thousands of other players in the Novaterra world.');
...
tz_def('TZ_NOW_YOU_KNOW_EVERYTHING_IMPORTANT', 'Now you know everything important about Novaterra. After registration you can start playing!');
...
tz_def('TZ_RESOURCES_BUILDINGS_VILLAGES_OR_TR', 'Resources, buildings, villages or troops lost during the time of suspension do not count as punishment and will not be replaced by the Novaterra Team. No player has the right to claim payment or replacement for Plus/Gold time lost due to suspension.');
...
tz_def('TZ_THERE_ARE_FOUR_DIFFERENT_TYPES_OF', 'There are four different types of resources in Novaterra: lumber, clay, iron and crop.');
tz_def('TZ_THERE_IS_NO_COMPENSATION_FOR_DAMAG', 'There is no compensation for damages done by a sitter. Account owners are fully responsible for the actions done by the chosen sitters of their account. In the event that sitters of an account do not follow these rules and the General Terms and Conditions of Novaterra, both the account owner and the sitter may be held responsible and punished.');
tz_def('TZ_THERE_IS_NO_COMPENSATION_FOR_DAMAG_2', 'There is no compensation for damages done by someone knowing the password of an account. The person receiving the password is subjected to the rules of Novaterra as well as the General Terms and Conditions.');
tz_def('TZ_THERE_IS_NO_SPECIAL_TREATMENT_FOR', 'There is no special treatment for Novaterra Plus/Gold users regarding the game rules neither in the time needed to deal with the case nor in the punishment.');
...
tz_def('TZ_THE_NOVATERRA_TEAM_RESERVES_THE_RIGH', 'The Novaterra Team reserves the right to change the rules at any time.');
...
tz_def('TZ_ML_CHANGELOG_120BUGS', 'Over 120 bugs fixed, artifacts fully fixed, catapults and rams fully fixed, automated Tatars/Artifacts/WW villages/WW building plans, new battle formula (more precise than the old one), automatic artifacts activation, rewritten a lot of code. See more in readme file!');
tz_def('TZ_ML_CHANGELOG_NEWFORUM', 'New forum system, Novaterra-like trapper formula, fixed master builder, double research queue in blacksmith and armoury with plus');
...
tz_def('TZ_ML_GPACK_ALTER_APPEARANCE', 'With a graphic pack you can alter the appearance of Novaterra. You can choose one from the list or provide a custom path.');
...
tz_def('TZ_ML_ACCOUNT_FROZEN', '. During this time your account is essentially frozen. No resources, troops or research will progress and your villages cannot be attacked. Remember, this just freezes your Novaterra, not time.');
...
tz_def('MANUAL_DESC_GREATWAREHOUSE', "The resources lumber, clay and iron are stored in your warehouse. The great warehouse offers you more space and keeps your resources drier and safer than the normal warehouse.<br /><br />This building can only be built in the old tatarian villages or with special tatarian artefacts.");
tz_def('MANUAL_DESC_GREATGRANARY', "The crop produced by your farms is stored in the granary. The great granary offers you more space and keeps your crops drier and safer than the normal one.<br /><br />This building can only be built in the old tatarian villages or with special tatarian artefacts.");
tz_def('MANUAL_DESC_WONDER', "The wonder of the world represents the pride of creation. Only the mightiest and richest are able to build such a masterwork and defend it against envious enemies.<br /><br />Wonders of the world can only be erected in the old tatarian villages. Also a construction plan is necessary. Starting with level 50 an additional plan is needed. This one has to be owned by another player in the same alliance.");
...
tz_def('MANUAL_NPC_NATARS', "Description is intended for reference only. The Tatars are a pure NPC tribe and thus cannot be played by oneself.");
...
tz_def('MANUAL_UDESC_11', "Clubswingers are the cheapest unit in Novaterra. They are quickly trained and have medium attack capabilities but their armour isn’t the best. Clubswingers are almost defenceless against cavalry and will be ridden down with ease.");
...
tz_def('MANUAL_UDESC_44', "The Tatars use flocks of birds to gather intelligence about their enemies. Thanks to the advantage of scouting from the air, it is almost impossible to stop the Tatarian scouting squads; on the other hand, even a simple-minded villager can easily notice the screeching and feathered flocks.");
tz_def('MANUAL_UDESC_41', "Their long and pointed pikes are used as the main line of defence in any battle. The Tatarian Pikemen are bold and daring warriors who use their dexterity to quickly down enemy horsemen and finish them off.");
tz_def('MANUAL_UDESC_42', "The thorn-like extensions on their helmets, bracers and shoulder-parts of their armour give the Thorned Warriors their name. The men who fight for the Tatars as Thorned Warriors are persistent and well trained, offering a bloody battle to anyone who is foolish enough to attack them.");
tz_def('MANUAL_UDESC_43', "Adored by their people and feared by their enemies, a Guardsman fights without a mount but is nevertheless one of the most valuable soldiers in the Tatarian army, thanks to his versatility. They are deemed as well trained fighters, leaving their enemies with almost no chances to win. Due to their heavy armour, they can be used as strong and reliable defence troops too.");
...
tz_def('MANUAL_UDESC_46', "Only the most skilful and strongest warriors of the Tatars survive the training to become a Tatarian Knight. Seeing them fight fills one with awe and shows what true warfare is. They wield their blades as if they were one with their arms and hands and use their shields seemingly as a natural extension of their bodies. Even the horses they ride are specially bred and trained - no normal horse would be able to wear the armour the knights' horses wear, let alone the knight himself, and still be able to go to war. Whispers of their glory have even reached the most distant kingdoms, spreading fear and horror.");
tz_def('MANUAL_UDESC_47', "No other tribe but the Tatars knows how to use these impressive creatures for their purposes. Neither a wall nor a palisade can withstand the War elephant’s attacks. A walking killing machine, trampling down anything that tries to oppose it or tries to come its way.");
tz_def('MANUAL_UDESC_48', "Even as engineers, the Tatars were very successful. They created machinery of war long before anyone else and have since then perfected it in every way. The Ballista, a huge crossbow-like weapon, fires its projectiles with such a force that no wall or shield can deflect them. When the engineers dismantle it to move it to the next battlefield, here is usually nothing but ruins left where the projectiles hit.");
tz_def('MANUAL_UDESC_49', "A mixture of pure fear, admiration and awe moves the villagers when the Tatarian Emperor speaks to them. This commanding and well outfitted figure is fully aware of his effect on others and knows how to subjugate an entire village with a single harangue.");
tz_def('MANUAL_UDESC_50', "Daring journeymen and master builders, driven by zest for action and knowing every little secret about cultivating land, building Palaces and fortifying villages, the Tatarian Settlers go out in parties of three to claim land in the name of their Tatarian lords.");
...
tz_def('MANUAL_NF_T_21', "Manual on Nature and Tatars");
...
tz_def('MANUAL_NF_D_11', "If there is a captured oasis in the village, it will be displayed in the player's profile in front of the corresponding village with the appropriate type of resource and a bonus for production. This function was presented in Novaterra T4.");
...
tz_def('MANUAL_NF_D_17', "If there is an artifact in one of the villages, this will be displayed in the player's profile in front of the corresponding village in which it is located. This function was presented in Novaterra T4.");
tz_def('MANUAL_NF_D_18', "If in the village there is a playground for the construction of the Wonder of the World, this will be displayed in the profile opposite the corresponding village. This function was presented in Novaterra T4.");
tz_def('MANUAL_NF_D_19', "Vacation mode allows you to protect your empire from any hostile actions of other players during your long absence. True, for this there are certain conditions that will entail a backlog in the development of your empire. This function was presented in Novaterra T4.");
tz_def('MANUAL_NF_D_20', "If you send the catapults to a normal attack, you can see in the rally point which targets you set for the attack. This function was presented in Novaterra T4.");
tz_def('MANUAL_NF_D_21', "With the help of this information in the manual, you can find a description of the forces of Nature and Tatars.");
tz_def('MANUAL_NF_D_22', "The location of the Direct links is changing. In the original Novaterra T3.6 Direct links are placed in the right menu below the list of villages which is not entirely convenient. If this option is enabled then Direct links will be placed in the left menu that is much more convenient.");
tz_def('MANUAL_NF_D_23', "The medal of which is awarded to players losing by the same email address of 3 or more years. Can be added to the profile description. This function was presented in Novaterra T4.");
tz_def('MANUAL_NF_D_24', "The medal of which is awarded to players losing by the same email address of 5 or more years. Can be added to the profile description. This function was presented in Novaterra T4.");
tz_def('MANUAL_NF_D_25', "The medal of which is awarded to players losing by the same email address of 10 or more years. Can be added to the profile description. This function was presented in Novaterra T4.");
tz_def('MANUAL_NF_D_26', "The medal of which is awarded to players losing by the same email address of 10 or more years. Can be added to the profile description. This function was presented in Novaterra T4.");
...
tz_def('HB_TXT_VS_NATARS',  'strength vs Tatars');
...
tz_def('ADMIN_NATARS_MANAGEMENT', 'Tatars Management');
...
tz_def('ADM_CG_FREE_GOLD_SETTING', 'Free Gold (X-Tatar)');
tz_def('ADM_CG_FREE_GOLD_DESC', 'Controls whether players can receive free gold credited from X-Tatar.com activity. This does not affect paid gold, which is always active once configured.');
tz_def('ADM_CG_FREE_GOLD_ENABLED', 'Enabled &ndash; players receive free gold from X-Tatar activity');
tz_def('ADM_CG_FREE_GOLD_DISABLED', 'Disabled &ndash; free gold is hidden/paused for all players');
tz_def('ADM_CG_SAVE', 'Save');
tz_def('ADM_CG_LOOKUP_TITLE', 'Look Up Player');
tz_def('ADM_CG_LOOKUP_DESC', 'Search by the email the player registered with to see their portable paid-gold balance and which worlds they have played on.');
tz_def('ADM_CG_EMAIL', 'Email');
tz_def('ADM_CG_SEARCH', 'Search');
tz_def('ADM_CG_NO_ACCOUNT_FOUND', 'No central gold account found for this email.');
tz_def('ADM_CG_PAID_BALANCE', 'Paid Gold Balance');
tz_def('ADM_CG_USERNAME', 'Username');
tz_def('ADM_CG_WORLDS_SEEN', 'Worlds Seen On');
tz_def('ADM_CG_NO_WORLDS', 'Not linked to any world yet.');
tz_def('ADM_CG_LAST_SEEN', 'Last Seen');
tz_def('ADM_CG_GRANT_TITLE', 'Grant Paid Gold');
tz_def('ADM_CG_GRANT_DESC', 'Credit paid gold straight to a player, identified by email &mdash; no source account needed. Use this instead of editing paid_gold directly in phpMyAdmin, so the ledger stays consistent and this shows up as an "admin_grant" entry below.');
tz_def('ADM_CG_GRANT_BTN', 'Grant Gold');
tz_def('ADM_CG_TRANSFER_TITLE', 'Transfer Paid Gold');
tz_def('ADM_CG_TRANSFER_DESC', 'Move paid gold from one player to another, identified by email, across any world. Only paid gold is transferable &mdash; free gold always stays local to the world it was earned on.');
tz_def('ADM_CG_FROM_EMAIL', 'From (Email)');
tz_def('ADM_CG_TO_EMAIL', 'To (Email)');
tz_def('ADM_CG_AMOUNT', 'Amount');
tz_def('ADM_CG_NOTE_OPTIONAL', 'Note (optional)');
tz_def('ADM_CG_TRANSFER_BTN', 'Transfer');
tz_def('ADM_CG_RECENT_LEDGER', 'Recent Ledger (All Worlds)');
tz_def('ADM_CG_NO_LEDGER', 'No ledger entries yet.');
tz_def('ADM_CG_WHEN', 'When');
tz_def('ADM_CG_PLAYER', 'Player');
tz_def('ADM_CG_WORLD', 'World');
tz_def('ADM_CG_DELTA', 'Change');
tz_def('ADM_CG_BALANCE_AFTER', 'Balance After');
tz_def('ADM_CG_REASON', 'Reason');
tz_def('ADM_CG_SEARCH_LABEL', 'Or search by username');
tz_def('ADM_CG_SEARCH_PLACEHOLDER', 'Start typing a username...');
tz_def('ADM_CG_SEARCH_NO_RESULTS', 'No matching players found.');
tz_def('ADM_CG_SEARCH_MIN_CHARS', 'Type at least 2 characters to search.');
...
tz_def('ADMIN_FEEDING_SYSTEM', 'Linked Accounts (Feeding)');
tz_def('ADM_FS_INTRO', 'Lets a player declare one or more of their own accounts on this world as "linked" (fed) accounts. Raiding a linked account ignores warehouse/cranny loot protection, so the full stock is lootable (limited only by the attacking troops\' own carry capacity). This is independent from Multi-Account Detection below, which only flags suspicious pairs for manual review and never changes gameplay.');
tz_def('ADM_FS_MULTIACC_NOTE', 'This is a different system from Multi-Account Detection. That tool only scores suspicious pairs for a human to review — it never blocks anyone or changes how raids work. This tool does change raid behaviour, but only between accounts an admin or player has explicitly linked here.');
tz_def('ADM_FS_SETTINGS_TITLE', 'Settings');
tz_def('ADM_FS_SETTINGS_DESC', 'Turn the feature on/off and set how many accounts each player is allowed to self-link.');
tz_def('ADM_FS_ENABLED', 'Enabled');
tz_def('ADM_FS_DISABLED', 'Disabled');
tz_def('ADM_FS_MAX_LINKED', 'Max linked accounts per player');
tz_def('ADM_FS_ANNOUNCED', 'This has been announced in the server rules');
tz_def('ADM_FS_ANNOUNCE_WARNING', 'It is strongly recommended to announce this system in your public server rules before enabling it, so players understand why some accounts can be raided without the usual loot cap.');
tz_def('ADM_FS_SAVE', 'Save');
tz_def('ADM_FS_ADD_TITLE', 'Link Accounts Manually');
tz_def('ADM_FS_ADD_DESC', 'Link two accounts by username directly from the admin panel. Links added here bypass each player\'s self-service cap.');
tz_def('ADM_FS_OWNER_USERNAME', 'Main account (raider)');
tz_def('ADM_FS_LINKED_USERNAME', 'Linked account (fed)');
tz_def('ADM_FS_LINK_BTN', 'Link');
tz_def('ADM_FS_ALL_LINKS_TITLE', 'All Linked Pairs');
tz_def('ADM_FS_NO_LINKS', 'No accounts linked yet.');
tz_def('ADM_FS_ADDED', 'Added');
tz_def('ADM_FS_ADDED_BY', 'Added By');
tz_def('ADM_FS_BADGE_ADMIN', 'Admin');
tz_def('ADM_FS_BADGE_SELF', 'Player');
...
tz_def('FS_PLAYER_DISABLED_NOTICE', 'This feature is currently turned off by the server admin.');
tz_def('FS_PLAYER_LIMIT_NOTICE', 'You can link up to %d account(s). You currently have %d linked.');
tz_def('FS_PLAYER_USERNAME_LABEL', 'Username to link');
tz_def('FS_PLAYER_ADD_BTN', 'Add Link');
tz_def('FS_PLAYER_NO_LINKS', 'You have not linked any accounts yet.');
tz_def('FS_PLAYER_REMOVE', 'Remove');
tz_def('FS_PLAYER_CONFIRM_REMOVE', 'Remove this linked account?');
tz_def('FS_PLAYER_LINKED_SINCE', 'Linked since');
tz_def('FS_ERR_FEATURE_DISABLED', 'This feature is currently disabled by the server admin.');
tz_def('FS_ERR_CANNOT_LINK_SELF', 'You cannot link your own account to itself.');
tz_def('FS_ERR_ACCOUNT_NOT_FOUND', 'No account with that username was found.');
tz_def('FS_ERR_ALREADY_LINKED', 'That account is already linked.');
tz_def('FS_ERR_LIMIT_REACHED', 'You have reached your maximum number of linked accounts.');
tz_def('FS_ERR_INVALID_ACCOUNT', 'Invalid account.');
tz_def('FS_ERR_GENERIC', 'Something went wrong — please try again.');
tz_def('FS_SUCCESS_ADDED', 'Account linked successfully.');
tz_def('FS_SUCCESS_REMOVED', 'Link removed.');


// X-Tatar activity-based free gold (see GameEngine/XTatarGold.php)
tz_def('ADM_XG_TITLE', 'X-Tatar Free Gold');
tz_def('ADM_XG_INTRO', 'Players earn FREE gold based on their activity (in-game daily login, or X-Tatar.com activity via webhook). This gold is local to this world and does not transfer to other worlds.');
tz_def('ADM_XG_SETTINGS_TITLE', 'Settings');
tz_def('ADM_XG_SETTINGS_DESC', 'Turn the feature on/off and control how activity points convert into gold.');
tz_def('ADM_XG_ENABLED', 'Enabled');
tz_def('ADM_XG_DISABLED', 'Disabled');
tz_def('ADM_XG_POINTS_PER_GOLD', 'Points per 1 Gold');
tz_def('ADM_XG_DAILY_LOGIN_POINTS', 'Daily Login Points');
tz_def('ADM_XG_DAILY_CAP_POINTS', 'Daily Cap (0 = unlimited)');
tz_def('ADM_XG_WEBHOOK_SECRET', 'Webhook Secret');
tz_def('ADM_XG_WEBHOOK_SECRET_PLACEHOLDER', 'leave blank to disable the webhook');
tz_def('ADM_XG_WEBHOOK_HELP', 'X-Tatar.com should POST to xtatar_gold_webhook.php with this secret to award points for off-game activity. The webhook rejects all requests until a secret is set here.');
tz_def('ADM_XG_SAVE', 'Save Settings');
tz_def('ADM_XG_ADJUST_TITLE', 'Manually Adjust Player Points');
tz_def('ADM_XG_ADJUST_DESC', 'Add or remove activity points for a specific player (e.g. to correct a webhook gap). A positive amount that completes a full gold is converted immediately.');
tz_def('ADM_XG_USERNAME', 'Username');
tz_def('ADM_XG_POINTS_DELTA', 'Points (+/-)');
tz_def('ADM_XG_NOTE_OPTIONAL', 'Note (optional)');
tz_def('ADM_XG_ADJUST_BTN', 'Apply');
tz_def('ADM_XG_NO_PLAYER_FOUND', 'No player found with that username.');
tz_def('ADM_XG_CURRENT_POINTS', 'Current Points');
tz_def('ADM_XG_PLAYER_HISTORY', 'Player History');
tz_def('ADM_XG_NO_HISTORY', 'No history yet.');
tz_def('ADM_XG_TOP_EARNERS', 'Top Earners');
tz_def('ADM_XG_NO_DATA', 'No data yet.');
tz_def('ADM_XG_TOTAL_EARNED', 'Total Points Earned');
tz_def('ADM_XG_TOTAL_GOLD_CONVERTED', 'Total Gold Converted');
tz_def('ADM_XG_RECENT_LOG', 'Recent Activity (All Players)');
tz_def('ADM_XG_WHEN', 'When');
tz_def('ADM_XG_TYPE', 'Type');
tz_def('ADM_XG_POINTS', 'Points');
tz_def('ADM_XG_GOLD', 'Gold');
tz_def('ADM_XG_SOURCE', 'Source');
tz_def('ADM_XG_TYPE_POINTS_AWARDED', 'Points awarded');
tz_def('ADM_XG_TYPE_GOLD_CONVERTED', 'Gold converted');
tz_def('ADM_XG_TYPE_ADMIN_ADJUST', 'Admin adjustment');
tz_def('ADM_XG_SOURCE_DAILY_LOGIN', 'Daily login');
tz_def('ADM_XG_SOURCE_WEBHOOK', 'X-Tatar.com activity');
tz_def('ADM_XG_SOURCE_ADMIN', 'Admin adjustment');
tz_def('ADM_XG_SOURCE_OTHER', 'Other');
tz_def('ADM_XG_NOTE_WEBHOOK', 'X-Tatar.com activity webhook');
tz_def('ADM_XG_NOTE_COLUMN', 'Note');
...
tz_def('ADMIN_PANEL_TITLE', 'Admin Panel');
tz_def('ADMIN_LOGGED', 'Logged');
tz_def('ADMIN_NOT_LOGGED_IN', 'Not Logged in');
tz_def('ADMIN_ROLE_ADMIN', 'Admin');
tz_def('ADMIN_ROLE_USER', 'User');
tz_def('ADMIN_PUNISHMENTS', 'Punishments (Mute / Market / Army)');
tz_def('ADMIN_PUNISHMENTS_SHORT', 'Punishments');
tz_def('ADMIN_CHAT_MODERATION', 'Chat Moderation');
tz_def('ADMIN_EDIT_TROOPS_TITLE', 'Edit troops');
tz_def('ADMIN_DELETE_ALLIANCE_CONFIRM', 'Delete alliance?');
tz_def('ADMIN_T4_HERO_CONTROLS_TITLE', 'T4 Hero controls');
tz_def('ADMIN_T4_RUNNING_ADVENTURE_NOTE', 'arrives (do not delete running adventures).');
tz_def('ADMIN_SERVER_GRAPHIC_PACK', 'Server graphic pack');
tz_def('ADMIN_DATE', 'Date');
tz_def('ADMIN_TIME', 'Time');
tz_def('ADMIN_TRUE', 'True');
tz_def('ADMIN_FALSE', 'False');
tz_def('ADMIN_ENABLED', 'Enabled');
tz_def('ADMIN_DISABLED', 'Disabled');
tz_def('ADMIN_LIMIT_MAILBOX_DISABLED', 'Limit mailbox disabled');
tz_def('ADMIN_FEATURE_FLAGS_TITLE', 'Feature Flags');
tz_def('ADMIN_FEATURE_FLAGS_INTRO', 'Add or remove on/off switches for any feature here, without needing a code or config.php change. Flags apply the moment you toggle them - no server restart or config regeneration involved. This is separate from the Server / PLUS / Log settings pages: those control fixed, built-in switches; this page is for free-form flags you define yourself.');
tz_def('ADMIN_ADD_FLAG', 'Add a new flag');
tz_def('ADMIN_KEY', 'Key');
tz_def('ADMIN_LABEL', 'Label');
tz_def('ADMIN_NOTE_OPTIONAL', 'Note (optional)');
tz_def('ADMIN_ENABLED', 'Enabled');
tz_def('ADMIN_ADD_FLAG_BUTTON', 'Add Flag');
tz_def('ADMIN_NO_FEATURE_FLAGS', 'No feature flags yet. Add one above.');
tz_def('ADMIN_STATUS', 'Status');
tz_def('ADMIN_UPDATED', 'Updated');
tz_def('ADMIN_ON', 'ON');
tz_def('ADMIN_OFF', 'OFF');
tz_def('ADMIN_DISABLE', 'Disable');
tz_def('ADMIN_ENABLE', 'Enable');
tz_def('ADMIN_DELETE_FLAG_CONFIRM', 'Delete this flag? Any code checking it will fall back to its default value.');
tz_def('ADMIN_FLAG_NOTE_PLACEHOLDER', 'What this flag controls');
tz_def('ADMIN_FLAG_HINT', 'Keys are lowercased and normalised to letters, numbers and underscores. Check a flag in code with FeatureFlags::isEnabled(\'your_key\').');
tz_def('ADMIN_ACCESS_DENIED', 'Access denied.');
tz_def('ADMIN_FEATURE_FLAGS', 'Feature Flags');
tz_def('ADMIN_ADD_REMOVE_FLAGS_INTRO', 'Add or remove on/off switches for any feature here, without needing a code or config.php change. Flags apply the moment you toggle them - no server restart or config regeneration involved. This is separate from the Server / PLUS / Log settings pages: those control fixed, built-in switches; this page is for free-form flags you define yourself.');
tz_def('ADMIN_GRAPHIC_PACK_INFO', 'The graphic pack every player sees by default. Packs are read from the gpack/ folder &mdash; a folder counts as a pack when it contains novaterra.css. Changing this switches the whole server\'s look.');
tz_def('ADMIN_DELETE', 'Delete');
tz_def('ADMIN_MESSAGE_SENT', 'Message sent successfully');
tz_def('ADMIN_NORMAL_USER', 'Normal user');
tz_def('ADMIN_NO_SITTER', 'No Sitter');
tz_def('ADMIN_EDIT_FAILED', 'Edit failed.');
tz_def('ADMIN_AVERAGE_RANK', 'Average Rank');
tz_def('ADMIN_DELETE_ALL', 'Delete All');
tz_def('ADMIN_PLAYERS_REPORTS', 'Players Reports');
tz_def('ADMIN_FOUND_PLAYERS', 'Found players');
tz_def('ADMIN_EDIT', 'Edit');
tz_def('ADMIN_NOT_FOUND', 'Not found');
tz_def('ADMIN_BACK', 'Back');
tz_def('ADMIN_EDIT_ACCESS', 'Edit access');
tz_def('ADMIN_ACCESS_CHANGED', 'Players Access Changed');
tz_def('ADMIN_CANNOT_CHANGE_OWN_ACCESS', 'you cannot change your own access');
tz_def('ADMIN_PLAYER_NOT_FOUND', 'Player not found.');
tz_def('ADMIN_GO_BACK', 'Go Back');
tz_def('ADMIN_FOUND_ALLIANCES', 'Found alliances');
tz_def('ADMIN_FOUND_EMAILS', 'Found emails');
tz_def('ADMIN_FOUND_IPS', 'Found IPs');
tz_def('ADMIN_NO_LOGIN_RECORDS', 'No login records found.');
tz_def('ADMIN_FOUND', 'found');
tz_def('ADMIN_INSTRUCTIONS', 'instructions');
tz_def('ADMIN_CONTACT_SUPPORT', 'Tried everything to no avail? Write an answer to this message to contact the Support');
tz_def('ADMIN_ACTIVE_SERVER_FEATURES', 'Active Server Features');
tz_def('ADMIN_NOW', 'now');
tz_def('ADMIN_LAST_ACTION', 'Last action');
tz_def('ADMIN_JUST_NOW', 'just now');
tz_def('ADMIN_MIN_AGO', 'min ago');
tz_def('ADMIN_ALL_FILTER', 'All');
tz_def('ADMIN_ATTACKS', 'Attacks');
tz_def('ADMIN_DEFENCES', 'Defences');
tz_def('ADMIN_SCOUTS', 'Scouts');
tz_def('ADMIN_TRADES', 'Trades');
tz_def('ADMIN_PLAYERS_MESSAGES', 'Players Messages');
tz_def('ADMIN_SYSTEM', 'System');
tz_def('ADMIN_NO_SUBJECT', '(no subject)');
tz_def('ADMIN_MAP_TITLE', 'Map');
tz_def('ADMIN_PLAYER', 'Player');
tz_def('ADMIN_COORD', 'Coord');
tz_def('ADMIN_POP', 'Pop');
tz_def('ADMIN_ARTIFACT', 'Artifact');
tz_def('ADMIN_PROFILE', 'Profile');
tz_def('ADMIN_NO_UNACTIVATED_PLAYERS', 'No unactivated players');
...
tz_def('ANL_ROMAN_DESC_1', 'Because of its high level of social and technological development the Romans are masters at building and its coordination. Also, their troops are part of the elite in Novaterra. They are very balanced and useful in attacking and defending.');
...
tz_def('ANL_TRIBES_INTRO', 'You can choose between three tribes in Novaterra: Romans, Gauls and Teutons. Each tribe has advantages and disadvantages and their troops are also very different. It is important to choose the tribe that fits you perfectly.');
...
tz_def('ANL_TEUTON_PLUNDER_BONUS', 'Plunder bonus: enemy crannies only hides 66% of the normal amount in Novaterra version 2.5 and 3, and 80% of the normal amount in Novaterra version 3.5.');
...
tz_def('ANL_TEUTON_CRANNY', 'Teutons: Enemy crannies only hides 66% of the normal amount in Novaterra version 2.5 and 3, and 80% of the normal amount in Novaterra version 3.5.');
...
tz_def('ANL_ROMAN_TRADE_OFFICE', 'Romans: In Novaterra version 3.5 the Roman trade office increases the capacity by 20% per level.');
...
tz_def('PUBLIC_UNOFFICIAL_PROJECT', 'This is an unofficial fan-made project and is not affiliated with Novaterra GmbH.');
...
tz_def('PUBLIC_TERMS_PROJECT', 'This is a fan-made open-source project called Novaterra by Shadow.');
...
tz_def('PUBLIC_TERMS_NO_AFFILIATION', 'This project is not affiliated with Novaterra.');
...
tz_def('PUBLIC_RULES_INTRO_1', 'The below set of rules are in addition to the General Terms and Conditions of Novaterra. You should familiarize yourself with the General Terms and Conditions to verify what is allowed, especially in the case of an account that has been banned for a rule violation.');
tz_def('PUBLIC_RULES_INTRO_2', 'Inciting, manipulating, encouraging, assisting or conspiring with others to violate any of Novaterra Rules is impermissible. These rules apply for players that will delete their accounts or are currently deleting their accounts without exception.');
...
tz_def('PUBLIC_RULES_PASSWORD_DAMAGE', 'There is no compensation for damages done by someone knowing the password for an account. The person receiving the password is subject to the rules of Novaterra as well as the General Terms and Conditions.');
...
tz_def('PUBLIC_RULES_SAME_WORLD_TRANSFER', 'In order to switch an account with a person on the same game world, both persons must send an e-mail message to admin@novaterra.com from the e-mail address currently registered for the account. The e-mail must contain the following information:');
...
tz_def('PUBLIC_RULES_SITTER_DAMAGE', 'There is no compensation for damages done by a sitter. Owners of an account are fully responsible for the actions of any sitters for their account. In the event that sitters of an account do not follow these rules and the General Terms and Conditions of Novaterra, both the account owner and the sitter may be punished.');
...
tz_def('PUBLIC_RULES_BLACKMAIL', 'Blackmailing players in a way that violates any of Novaterra\'s rules respective the general terms and conditions.');
tz_def('PUBLIC_RULES_ADVERTISING', 'Advertisement of any kind that has not been permitted by the Novaterra Team is impermissible.');
...
tz_def('PUBLIC_RULES_NO_REPLACEMENT', 'Resources, buildings, villages or troops lost during the time of suspension do not count as punishment and will not be replaced by the Novaterra Team. No player has the right to claim payment or replacement for Plus/Gold time lost due to suspension.');
tz_def('PUBLIC_RULES_NO_SPECIAL_TREATMENT', 'here is no special treatment for Novaterra Plus/Gold users regarding the game rules neither in the time needed to deal with the case nor in the punishment.');
...
tz_def('PUBLIC_RULES_OWNER_INFORMATION', 'Additionally, the Novaterra Team will not provide information to persons other than the account owner.');
...
tz_def('ADM_PUN_TITLE', 'Punishments (Mute / Market / Army Freeze)');
tz_def('ADM_PUN_MUTED_NOW', 'Muted now');
tz_def('ADM_PUN_MARKET_NOW', 'Market-restricted now');
tz_def('ADM_PUN_ARMY_NOW', 'Army-frozen now');
tz_def('ADM_PUN_APPLY_TITLE', 'Apply Restriction');
tz_def('ADM_PUN_USER_ID', 'User ID');
tz_def('ADM_PUN_TYPE_MUTE', 'Mute (chat)');
tz_def('ADM_PUN_TYPE_MARKET', 'Market restriction');
tz_def('ADM_PUN_TYPE_ARMY', 'Army freeze');
tz_def('ADM_PUN_REASON_PH', 'Reason (required, shown to the player)');
tz_def('ADM_PUN_APPLY_BTN', 'Apply Restriction');
tz_def('ADM_PUN_LABEL_MUTE', 'chat mute');
tz_def('ADM_PUN_LABEL_MARKET', 'marketplace');
tz_def('ADM_PUN_LABEL_ARMY', 'army freeze');
tz_def('ADM_PUN_ACTIVE', 'Active');
tz_def('ADM_PUN_NO_ACTIVE', 'No active %s restrictions');
tz_def('ADM_PUN_CONFIRM_LIFT', 'Lift this restriction?');
tz_def('ADM_PUN_ERR_INVALID_UID', 'Invalid User ID!');
tz_def('ADM_PUN_ERR_SYSTEM_ACC', 'You cannot restrict system accounts!');
tz_def('ADM_PUN_ERR_INVALID_TYPE', 'Invalid restriction type!');
tz_def('ADM_PUN_ERR_REASON_REQUIRED', 'A reason is required!');
tz_def('ADM_PUN_ERR_NO_USER', 'This user does not exist!');
tz_def('ADM_PUN_SUCCESS_APPLY', 'Applied <b>%s</b> restriction to <b>%s</b>.');
tz_def('ADM_PUN_ERR_APPLY_FAILED', 'Could not apply this restriction!');
tz_def('ADM_PUN_ERR_INVALID_REQUEST', 'Invalid request.');
tz_def('ADM_PUN_LIFTED', 'Restriction lifted.');
tz_def('ADM_PUN_PERMANENT', 'Permanent (until lifted)');
tz_def('ADM_BAN_ERR_INVALID_UID', 'Invalid User ID!');
tz_def('ADM_BAN_ERR_SYSTEM_ACC', 'You cannot ban system accounts!');
tz_def('ADM_BAN_ERR_NO_USER', 'This user does not exist!');
tz_def('ADM_BAN_ERR_ALREADY_BANNED', 'User is already banned!');
tz_def('ADM_BAN_SUCCESS_USER', 'User <b>%s</b> has been banned successfully!');
tz_def('ADM_BAN_ERR_INVALID_IP', 'Invalid IP address!');
tz_def('ADM_BAN_SUCCESS_IP', 'IP <b>%s</b> has been banned successfully!');
tz_def('ADM_BAN_ERR_IP_FAILED', 'Could not ban this IP!');
tz_def('ADM_BAN_REASON_PUSHING', 'Pushing');
tz_def('ADM_BAN_REASON_CHEAT', 'Cheat');
tz_def('ADM_BAN_REASON_HACK', 'Hack');
tz_def('ADM_BAN_REASON_BUG', 'Bug');
tz_def('ADM_BAN_REASON_BADNAME', 'Bad Name');
tz_def('ADM_BAN_REASON_MULTI', 'Multi Account');
tz_def('ADM_BAN_REASON_SWEARING', 'Swearing');
tz_def('ADM_HOUR_UNIT', 'hour/s');
tz_def('ADM_DAY_UNIT', 'day/s');
tz_def('ADM_NO_ACTIVE_BANS', 'No active bans');
tz_def('ADM_NO_ACTIVE_IP_BANS', 'No active IP bans');
tz_def('ADM_NO_BAN_HISTORY', 'No history');
tz_def('ADM_CONFIRM_UNBAN_USER', 'Unban this player?');
tz_def('ADM_CONFIRM_UNBAN_IP', 'Unban this IP?');
...
tz_def('ADM_CANCEL_VACATION', 'Cancel Vacation');
tz_def('ADM_CANCEL_VACATION_CONFIRM', 'Cancel Vacation Mode for this player?');
...
tz_def('ADM_MANUAL_ON_NATURE_AND_NATARS', 'Manual on Nature and Tatars');
tz_def('ADM_ENABLE_DISABLE_MANUAL_INFO', 'Enable (Disable) manual info');
tz_def('ADM_ENABLE_DISABLE_DISPLAYING_INFORMATION_IN_THE', 'Enable (Disable) displaying information in the Manual about the troops of Nature and Tatars');
tz_def('ADM_DIRECT_LINKS_PLACEMENT', 'Direct links placement');
tz_def('ADM_LEFT_MENU_VS_RIGHT_MENU', 'Left menu vs right menu');
tz_def('ADM_IF_ENABLED_THEN_THE_DIRECT_LINKS_WILL_BE_PLA', 'If Enabled, then the Direct links will be placed in the left menu, if Disabled then Direct links will be placed in the right menu as in the original Novaterra');
tz_def('ADM_MEDAL_VETERAN_PLAYER', 'Medal Veteran Player');
tz_def('ADM_3_YEARS', '3 years');
tz_def('ADM_ENABLE_DISABLE_MEDAL_ACHIEVED_FOR_PLAYING_3', 'Enable (Disable) medal achieved for playing 3 years of Novaterra');
tz_def('ADM_MEDAL_VETERAN_PLAYER_5A', 'Medal Veteran Player 5a');
tz_def('ADM_5_YEARS', '5 years');
tz_def('ADM_ENABLE_DISABLE_MEDAL_ACHIEVED_FOR_PLAYING_5', 'Enable (Disable) medal achieved for playing 5 years of Novaterra');
tz_def('ADM_MEDAL_VETERAN_PLAYER_10A', 'Medal Veteran Player 10a');
tz_def('ADM_10_YEARS', '10 years');
tz_def('ADM_ENABLE_DISABLE_MEDAL_ACHIEVED_FOR_PLAYING_10', 'Enable (Disable) medal achieved for playing 10 years of Novaterra');
...
tz_def('ADM_YES', 'Yes');
tz_def('ADM_NO', 'No');
tz_def('ADM_NOVATERRA_OFFICIAL', 'Novaterra Official');
tz_def('ADM_NOVATERRA_EXTENDED', 'Novaterra Extended');
...
tz_def('ADM_GOLD_LOCAL_ONLY_LABEL', 'Local only (do not sync to this player\'s cross-world gold balance)');
...
tz_def('ADM_ACTIVATE_NOVATERRA_PLUS_FOR_ALL_PLAYERS_ON_THE', 'Activate Novaterra Plus for ALL players on the server.');
...
tz_def('ADM_ADMIN_ACCOUNTS_ALWAYS_KEPT', 'Administrator accounts are always preserved');
tz_def('ADM_ADMIN_ACCOUNTS_ALWAYS_KEPT_HINT', 'Every account with admin access keeps the same login/password and receives a new starter village at the map center so you can log in and manage the server after the reset.');
...
tz_def('ADM_RETURN_TO_NATARS', 'Return to Tatars');
...
tz_def('ADM_MAD_ON', 'ON');
tz_def('ADM_MAD_OFF', 'OFF');
tz_def('ADM_MAD_AUTO_BAN', 'Auto-Ban');
tz_def('ADM_MAD_ENABLE_DETECTION', 'Multi-Account Detection');
tz_def('ADM_MAD_ENABLE_AUTO_BAN', 'Automatic Ban');
tz_def('ADM_MAD_AUTO_BAN_THRESHOLD', 'Auto-ban score threshold');
tz_def('ADM_MAD_AUTO_BAN_HINT', 'When both are ON, any pair reaching the threshold is banned automatically as soon as this page loads or these settings are saved.');
tz_def('ADM_MAD_AUTO_BAN_JUST_RAN', 'Auto-ban just ran');
tz_def('ADM_MAD_FULL_ADMIN_ONLY', 'Only a full administrator can change these settings.');
tz_def('ADM_MAD_DETECTION_DISABLED_NOTICE', 'Multi-Account Detection is currently OFF. Enable it above to scan for suspicious account pairs.');
tz_def('ADM_MAD_VIEW_PAIR', 'View');
tz_def('ADM_MAD_PLAYER_DASHBOARD', 'Multi Account Admin');
tz_def('ADM_MAD_BACK_TO_OVERVIEW', 'Back to Multi-Account Detection');
tz_def('ADM_MAD_BANNED', 'Banned');
tz_def('ADM_MAD_ACTIVE', 'Active');
tz_def('ADM_MAD_RELATED_PROTECTION_OFF_NOTICE', 'Related Account Protection is currently OFF — raiding between related accounts is not being blocked.');
tz_def('ADM_MAD_ACCOUNT_ACTIONS', 'Actions');
tz_def('ADM_MAD_BAN_ACCOUNT', 'Ban Account');
tz_def('ADM_MAD_UNBAN_ACCOUNT', 'Unban Account');
tz_def('ADM_MAD_BAN_REASON', 'Reason');
tz_def('ADM_MAD_RELATE_TO_UID', 'Related account UID');
tz_def('ADM_MAD_REASON_OPTIONAL', 'Reason (optional)');
tz_def('ADM_MAD_MARK_AS_RELATED', 'Mark as Related');
tz_def('ADM_MAD_SUSPECTED_PAIRS', 'Auto-Detected Suspicious Pairs');
tz_def('ADM_MAD_NO_SUSPECTED_PAIRS', 'No suspicious pairs detected for this account.');
tz_def('ADM_MAD_OTHER_ACCOUNT', 'Account');
tz_def('ADM_MAD_RELATED_ACCOUNTS', 'Manually Related Accounts');
tz_def('ADM_MAD_NO_RELATED_ACCOUNTS', 'No accounts manually related to this one.');
tz_def('ADM_MAD_REASON', 'Reason');
tz_def('ADM_MAD_ADDED', 'Added');
tz_def('ADM_MAD_REMOVE', 'Remove');
tz_def('ADM_MAD_IPS_AND_DEVICES', 'IPs & Devices');
tz_def('ADM_MAD_NO_FINGERPRINTS', 'No IP/device fingerprints recorded yet.');
tz_def('ADM_MAD_HITS', 'Hits');
tz_def('ADM_MAD_LAST_SEEN', 'Last Seen');
tz_def('ADM_MAD_BLOCKED_ATTEMPTS', 'Blocked Raid/Transfer Attempts');
tz_def('ADM_MAD_NO_BLOCKED_ATTEMPTS', 'No blocked attempts recorded for this account.');
tz_def('ADM_MAD_FROM', 'From');
tz_def('ADM_MAD_TO', 'To');
tz_def('ADM_MAD_RESOURCES', 'Resources');
tz_def('ADM_MAD_WHEN', 'When');
tz_def('ADM_MAD_RECENT_LOGINS', 'Recent Logins');
tz_def('ADM_MAD_NO_LOGIN_RECORDS', 'No login records found.');
tz_def('ADM_MAD_BAN_HISTORY', 'Ban History');
tz_def('ADM_MAD_NO_BAN_HISTORY', 'No ban history for this account.');
tz_def('ADM_MAD_STATUS', 'Status');
tz_def('ADM_MAD_LIFTED', 'Lifted');
...
tz_def('ADM_HEATMAP_ACCESS_DENIED', 'Access denied.');
tz_def('ADM_HEATMAP_MAP_RADIUS', 'Map radius');
tz_def('ADM_HEATMAP_GRID', 'grid');
tz_def('ADM_HEATMAP_TILES_CELL', 'tiles/cell');
tz_def('ADM_HEATMAP_VILLAGE_DENSITY', 'Village density');
tz_def('ADM_HEATMAP_TRIBE_DENSITY', 'Tribe density');
tz_def('ADM_HEATMAP_INACTIVITY', 'Inactivity');
tz_def('ADM_HEATMAP_ATTACKS', 'Attacks');
tz_def('ADM_HEATMAP_LEGEND', 'Legend');
tz_def('ADM_HEATMAP_VILLAGES', 'villages');
tz_def('ADM_HEATMAP_POPULATION', 'population');
tz_def('ADM_HEATMAP_INACTIVE', 'inactive');
tz_def('ADM_HEATMAP_DOMINANT', 'dominant');
tz_def('ADM_HEATMAP_CELL_DOMINANT', 'Cell shows the dominant tribe; brighter = more villages.');
tz_def('ADM_HEATMAP_HIGHER_COUNT', 'Darker/brighter cells = higher count in that region.');
...
tz_def('ADM_SECURE_ACCESS_TO_NOVATERRA_PANEL', 'Secure access to Novaterra panel');
...
tz_def('ADM_THIS_ACTION_WILL_DISABLE_NOVATERRA_PLUS_FOR_AL', 'This action will disable Novaterra Plus for ALL players.');
...
tz_def('ADM_USE_IT_TO_PRIORITISE_WHICH_PAIRS_A_HUMAN_SHO', '&mdash; use it to prioritise which pairs a human should investigate. Nothing is banned automatically unless the admin turns on Automatic Ban below.');
...
tz_def('PLUSSTATS_NEEDPLUS', 'Graphical statistics are a Novaterra Plus feature. Activate Plus to see the development of your account.');
...
tz_def('CONF_SERV_NATARS_WW_START_DELAY', 'Tatars\' World Wonder start');
tz_def('CONF_SERV_NATARS_WW_START_DELAY_TOOLTIP', 'How many days after the construction plans appear the Tatars start building their own World Wonder. Scaled by server speed. 0 disables it.');
...
tz_def('TZ_WS_VS_NATARS', 'Against Tatars');
...
tz_def('TZ_MANUAL_PLUS_INTRO', 'The plus account offers you the following improvements:');
tz_def('TZ_MANUAL_WAITING_LOOP', 'Waiting loop for constructions');
tz_def('TZ_MANUAL_LARGER_MAP', 'Larger map');
tz_def('TZ_MANUAL_ARCHIVE_FUNCTION', 'Archive function for reports and messages');
tz_def('TZ_MANUAL_SORT_REPORTS', 'Sorting function for reports and messages');
tz_def('TZ_MANUAL_SORT_MARKET', 'Sorting function for the marketplace');
tz_def('TZ_MANUAL_AUTOCOMPLETE', 'Auto-completion');
tz_def('TZ_MANUAL_DIRECT_LINKS_FREE', 'Freely definable direct links');
tz_def('TZ_MANUAL_GRAPHIC_STATS', 'Graphical statistics');
tz_def('TZ_MANUAL_CENTRAL_OVERVIEW', 'Central account overview');
tz_def('TZ_MANUAL_NOTEPAD', 'Notepad');
tz_def('TZ_MANUAL_BONUS_NOTE', 'The bonus will not be added to the single resource fields but to the sum of the production.');
tz_def('TZ_MANUAL_GOLD_CLUB_EXCLUDED', 'This feature is NOT included in the gold club!');
tz_def('TZ_MANUAL_BONUS_TITLE_LUMBER', 'Production bonus for lumber');
tz_def('TZ_MANUAL_BONUS_INTRO_LUMBER', 'With this Gold advantage all your villages\' lumber production will be increased by 25%.');
tz_def('TZ_MANUAL_BONUS_TITLE_CLAY', 'Production bonus for clay');
tz_def('TZ_MANUAL_BONUS_INTRO_CLAY', 'With this Gold advantage all your villages\' clay production will be increased by 25%.');
tz_def('TZ_MANUAL_BONUS_TITLE_IRON', 'Production bonus for iron');
tz_def('TZ_MANUAL_BONUS_INTRO_IRON', 'With this Gold advantage all your villages\' iron production will be increased by 25%.');
tz_def('TZ_MANUAL_BONUS_TITLE_CROP', 'Production bonus for crop');
tz_def('TZ_MANUAL_BONUS_INTRO_CROP', 'With this Gold advantage all your villages\' crop production will be increased by 25%.');
tz_def('TZ_MANUAL_INSTANT_TITLE', 'Complete construction orders &amp; research immediately.');
tz_def('TZ_MANUAL_INSTANT_DESC', 'In the current village all construction orders and research in the academy as well as the blacksmith and armoury will be completed immediately.');
tz_def('TZ_MANUAL_INSTANT_EXCLUDE_PREFIX', 'However, the <em>buildings</em> Residence and Palace and');
tz_def('TZ_MANUAL_INSTANT_EXCLUDE_SUFFIX', 'with a wonder of the world inside them are excluded.');
tz_def('TZ_MANUAL_NPC_TITLE', 'NPC Merchant');
tz_def('TZ_MANUAL_NPC_DESC', 'The NPC Merchant will exchange any desired amount of resources in a village with other resources at a ratio of 1:1.');
tz_def('TZ_MANUAL_GOLDCLUB_INTRO', 'You will only be charged once for entry into Gold club. Afterwards you have access to various exclusive features. Some of the exclusive features are free, others cost a defined amount of gold per use.');
tz_def('TZ_MANUAL_GOLDCLUB_FREE_HEADER', 'Free Gold club features:');
tz_def('TZ_MANUAL_RAID_LIST', 'Raid list');
tz_def('TZ_MANUAL_RAID_STATS', 'Raid statistics');
tz_def('TZ_MANUAL_MERCHANTS_THRICE', 'Let merchants go thrice');
tz_def('TZ_MANUAL_SEARCH_CROPPERS', 'Search for 9 and 15 croppers including oases');
tz_def('TZ_MANUAL_GOLDCLUB_PRICED_HEADER', 'Priced Gold club features:');
tz_def('TZ_MANUAL_MASTER_BUILDER_3', 'Assign master builder up to 3 construction orders');
tz_def('TZ_MANUAL_AUTO_TRADE', 'Automated trade routes between your own villages');
tz_def('TZ_MANUAL_HIDE_TROOPS', 'Hide troops from attacks in the forest');
tz_def('TZ_MANUAL_HIDE_TROOPS_NOTE', '(only possible if in the last <strong>10</strong> seconds no troops came home)');
tz_def('TZ_MANUAL_DIRECT_LINKS_DESC', 'With the direct links one can set links to any target and speed up the navigation.');
tz_def('TZ_MANUAL_DIRECT_LINKS_EXAMPLE_HEADER', 'example for direct links');
tz_def('TZ_MANUAL_DIRECT_LINKS_STAR_NOTE', 'Adding an * to the URL will cause the direct link to open in a new window.');
tz_def('ANL_ROMAN_TRAIT_INFANTRY', 'Very powerful infantry, average cavalry');
tz_def('ANL_ROMAN_TRAIT_DEV', 'Development is expensive and takes a long time.');
tz_def('ANL_GAUL_TRAIT_SIEGE', 'Expensive siege weapons');
tz_def('ANL_GAUL_TRAIT_SETTLERS', 'Cheap settlers');
tz_def('ANL_UNIT_SPEED_HEADER', 'Speed');
tz_def('ANL_FAQ_CROP_SINK_Q', 'Help, my crop production sinks more and more!');
tz_def('TZ_PLAYERS_ONLINE', 'Players online');
tz_def('TZ_SERVER_INFO', 'Server Information');
tz_def('TZ_DAYS_SINCE_START', 'Days Since Start');
tz_def('TZ_ACTIVE_ALLIANCE', 'Active Alliance');
tz_def('TZ_NATARS_VILLAGES', 'Natars Villages');
tz_def('TZ_CONQUERED_OASIS', 'Conquered Oasis');
tz_def('TZ_NEW_PLAYERS_TODAY', 'New Players Today');
tz_def('TZ_TOP_5_ALLIANCES', 'Top 5 Alliances');
tz_def('TZ_TOP_PLAYERS', 'Top Players');
tz_def('TZ_ENDGAME', 'Endgame');
tz_def('TZ_ATTACKERS', 'Attackers');
tz_def('TZ_DEFENDERS', 'Defenders');
tz_def('TZ_ARTEFACTS_LABEL', 'Artefacts');
tz_def('TZ_WW_SHORT', 'WW');
tz_def('TZ_NO_USERS_FOUND', 'No users found');
tz_def('TZ_NO_ALLIANCES_FOUND', 'No alliances found');
tz_def('TZ_NO_HEROES_FOUND', 'No heroes found');
tz_def('TZ_NO_VILLAGES_FOUND', 'No villages found');
tz_def('TZ_NO_MESSAGES_INBOX', 'There are no messages available.');
tz_def('TZ_NO_MESSAGES_SENT', 'There are no sent messages available.');
tz_def('TZ_NO_MESSAGES_ARCHIVE', 'There are no messages available in the archive.');
tz_def('TZ_REMOVE_SITTER', 'Remove sitters');
tz_def('TZ_ACCOUNT_DELETE_TIMER_PREFIX', 'The account will be deleted in');
tz_def('TZ_MEDAL_ATTACKER_WEEK', 'Attacker of the Week');
tz_def('TZ_MEDAL_DEFENDER_WEEK', 'Defender of the Week');
tz_def('TZ_MEDAL_POP_CLIMBER_WEEK', 'Pop Climber of the week');
tz_def('TZ_MEDAL_ROBBER_WEEK', 'Robber of the week');
tz_def('TZ_MEDAL_TOP10_ATT_DEF', 'Top 10 Attack+Def');
tz_def('TZ_MEDAL_TOP_ATTACK_STREAK', 'Top Attack streak');
tz_def('TZ_MEDAL_TOP_DEF_STREAK', 'Top Def streak');
tz_def('TZ_MEDAL_TOP_POP_STREAK', 'Top Pop streak');
tz_def('TZ_MEDAL_TOP_ROB_STREAK', 'Top Rob streak');
tz_def('TZ_MEDAL_RANK_CLIMBER', 'Rank Climber');
tz_def('TZ_MEDAL_RANK_STREAK', 'Rank streak');
tz_def('TZ_MEDAL_TOP10_ATTACK', 'Top 10 Attack');
tz_def('TZ_MEDAL_TOP10_DEF', 'Top 10 Def');
tz_def('TZ_MEDAL_TOP10_POP', 'Top 10 Pop');
tz_def('TZ_MEDAL_TOP10_ROB', 'Top 10 Rob');
tz_def('TZ_MEDAL_TOP10_RANK', 'Top 10 Rank');
tz_def('TZ_MEDAL_BONUS_DEFAULT', 'Bonus');
...
tz_def('TZ_CB_TITLE', 'Build crop finder');
tz_def('TZ_CB_PAGE_TITLE_SUFFIX', 'Build Croppers');
tz_def('TZ_CB_STATUS', 'Status');
tz_def('TZ_CB_WORLD', 'World');
tz_def('TZ_CB_9C15C_IN_MAP', '9c/15c in map');
tz_def('TZ_CB_ROWS_IN_TABLE', 'Rows in table');
tz_def('TZ_CB_LAST_UPDATED', 'Last updated');
tz_def('TZ_CB_TABLE_NOTE', 'The croppers table stores only <b>wref,x,y,fieldtype,best_oasis_bonus</b>. Ownership and occupied status are pulled live from <code>vdata/users</code> by the finder.');
tz_def('TZ_CB_ACTIONS', 'Actions');
tz_def('TZ_CB_BATCH_SIZE', 'Batch size');
tz_def('TZ_CB_BUILD_REBUILD', 'Build / Rebuild');
tz_def('TZ_CB_ESTIMATE', 'Estimate');
tz_def('TZ_CB_REINDEX', 'Reindex');
tz_def('TZ_CB_TRUNCATE', 'Truncate');
tz_def('TZ_CB_CONFIRM_TRUNCATE', 'Really truncate the table?');
tz_def('TZ_CB_STREAM_NOTE', 'Building streams progress below. You can leave this page; the process stops when the request ends.');
tz_def('TZ_CB_INVALID_CSRF', 'Invalid CSRF token. Please reload the page.');
tz_def('TZ_CB_TRUNCATED_NOTICE', 'Croppers table truncated.');
tz_def('TZ_CB_REINDEXED_NOTICE', 'Indexes rebuilt.');
tz_def('TZ_CB_ESTIMATED_NOTICE', 'Estimated counts refreshed.');
tz_def('TZ_CB_WORLD_UNKNOWN', 'unknown');
tz_def('TZ_CB_LOG_STARTED', 'Croppers builder started');
tz_def('TZ_CB_LOG_DETECTED', 'Detected %d croppers.');
tz_def('TZ_CB_LOG_QUERY_FAILED', 'Query failed: %s');
tz_def('TZ_CB_LOG_UPSERT_FAILED', 'Upsert failed: %s');
...
tz_def('ADM_DEBUG_WARNING', 'Warning');
tz_def('ADM_DEBUG_NOTICE', 'Notice');
tz_def('ADM_DEBUG_DEPRECATED', 'Deprecated');
tz_def('ADM_DEBUG_FATAL_ERROR', 'Fatal error');
tz_def('ADM_DEBUG_PHP_WARNING', 'PHP Warning');
tz_def('ADM_DEBUG_PHP_NOTICE', 'PHP Notice');
tz_def('ADM_DEBUG_PHP_DEPRECATED', 'PHP Deprecated');
tz_def('ADM_DEBUG_UNDEFINED_VARIABLE', 'Undefined variable');
tz_def('ADM_DEBUG_UNDEFINED_ARRAY_KEY', 'Undefined array key');
tz_def('ADM_DEBUG_UNDEFINED_CONSTANT', 'Undefined constant');
tz_def('ADM_DEBUG_FAILED_OPEN_STREAM', 'Failed to open stream');
...
// Admin: results_villages.tpl (added by AR localization phase 2)
tz_def('ADM_FOUND_VILLAGES', 'Found villages');
tz_def('ADM_VILLAGES_FOUND', 'Villages Found');
tz_def('ADM_NO_VILLAGES_CALLED', 'No Villages Called');

// Admin: massmessage.tpl (added by AR localization phase 2)
tz_def('ADM_IN_GAME_MESSAGE_ALL_USERS', 'In-game message for all users (ID > 5)');

// Admin: XTatarGold.php (added by AR localization phase 2)
tz_def('ADM_CONSECUTIVE_LOGIN_DAY', 'Consecutive login day');
tz_def('ADM_FIRST_LOGIN_DAY', 'First login of the day');

// Admin: editUser.tpl (added by AR localization phase 2)
tz_def('ADM_VILLAGES_LABEL', 'Villages');
tz_def('ADM_GOLD_LABEL', 'Gold');

// Admin: inactive.tpl (added by AR localization phase 2)
tz_def('ADM_1_3_DAYS', '1-3 days');
tz_def('ADM_3_7_DAYS', '3-7 days');
tz_def('ADM_7_PLUS_DAYS', '7+ days');
tz_def('ADM_NO_USERS_IN_RANGE', 'No users in this range');
tz_def('ADM_DAYS_AGO', 'd');
tz_def('ADM_HOURS_AGO', 'h ago');
tz_def('ADM_POPULATION_SHORT', 'pop');
tz_def('ADM_VILLAGES_SHORT', 'vil');
tz_def('ADM_DELETE_CONFIRM', 'Delete?');
