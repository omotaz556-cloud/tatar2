...
tz_def('PER_HR', 'لكل ساعة');
tz_def('PRODUCTION', 'الإنتاج');
tz_def('GK_LUMBER', 'الخشب');
tz_def('GK_CLAY', 'الطين');
tz_def('GK_IRON', 'الحديد');
tz_def('GK_CROP', 'القمح');
tz_def('GK_TROOPS', 'الوحدات');
tz_def('GK_NONE', 'لاشيء');
...
tz_def('TZ_UPGRADE_NAVIGATION', 'مكان العرض بعد الترقية');
tz_def('TZ_UPGRADE_NAVIGATION_DEFAULT', 'افتراضي: المستوى 1 على الخريطة، وبعده داخل نفس المبنى');
tz_def('TZ_UPGRADE_NAVIGATION_MAP', 'الخريطة دائمًا');
tz_def('TZ_UPGRADE_NAVIGATION_BUILDING', 'نفس المبنى دائمًا');
...
tz_def('ALL', 'الكل');
tz_def('NPC_TRADE', 'تاجر مبادله');
tz_def('NPC_TRADING', 'تاجر مبادله');
tz_def('BUY_AND_SELL', 'بيع وشراء');
tz_def('TRADE_RESOURCES', 'إتمام المبادلة');
tz_def('BUILD_LEVEL_SHORT', 'مستوى');
tz_def('SUM', 'المجموع');
tz_def('REST', 'الباقي');
tz_def('DISTRIBUTE_RESOURCES', 'توزيع المواد الخام (الخطوة 1 من 2)');
tz_def('OF', 'من');
tz_def('NPC_COMPLETED', 'اكتملت المبادلة');
tz_def('BACK_BUILDING', 'العودة إلى المبنى');
tz_def('YOU_CAN_NAT_NPC_WW', 'لا يمكنك استخدام تاجر المبادلة في قرية عجيبة الدنيا.');
...
tz_def("HEROSMANSION", "قصر الأبطال");
tz_def('HERO_READY', 'سيكون البطل جاهزًا خلال ');
tz_def('NAME_CHANGED', 'تم تغيير اسم البطل');
tz_def('NOT_UNITS', 'وحدات غير متاحة');
tz_def('NOT', 'ليس ');
tz_def('TRAIN_HERO', 'تدريب بطل');
tz_def('HERO_TYPE', 'نوع البطل');
tz_def('HERO_ACTIVITY', 'النشاط');
tz_def('HERO_NEW_STARTS', 'بطل جديد يبدأ عند مستوى 1');
tz_def('HERO_INFO_TITLE', 'معلومات البطل');
tz_def('HERO_LABEL_NAME', 'الاسم');
tz_def('HERO_LABEL_HEALTH', 'الصحة');
tz_def('HERO_LABEL_TYPE', 'النوع');
tz_def('HERO_LABEL_OFF_ARMY', 'هجوم الجيش بقيادة البطل');
tz_def('HERO_LABEL_DEF_ARMY', 'دفاع الجيش بقيادة البطل');
tz_def('HERO_LABEL_INDIVIDUAL', 'القوة الفردية للبطل');
tz_def('HERO_LABEL_REGEN', 'التجديد');
tz_def('HERO_LABEL_STATUS', 'حالة البطل');
tz_def('HERO_LABEL_CARRY', 'الحمولة عند النهب');
tz_def('HERO_LABEL_POINTS_NOW', 'نقاط البطل الآن');
tz_def('HERO_LABEL_POINTS_NEXT', 'نقاط البطل للمستوى %d');
tz_def('HERO_STATUS_READY', 'جاهز للدفاع عن القرية');
tz_def('HERO_STATUS_AWAY', 'البطل خارج القرية');
tz_def('HERO_STATUS_DEAD', 'البطل ميت');
tz_def('HERO_RES_UNIT', 'مورد');
tz_def('HERO_POINT_UNIT', 'نقطة');
tz_def('NO_HERO_UNITS', 'لا يوجد جنود لتدريب بطل، يجب تدريب اي جندي ليمكن ترقيته إلى بطل.');
tz_def('OASIS_OWNED_OF', 'لقد استوليت على %d من %d واحات وللمزيد طوّر المبنى');
tz_def('OASIS_RANGE_HINT', 'لايمكن إحتلال واحات خارج المربع 7 x 7 وزيادة السكان إلى %s توسع المربع');
tz_def('OASIS_VIEW_NEARBY', 'عرض الواحات القريبة منك التي يمكنك احتلالها');
tz_def('OASIS_EXPAND_POP', '1020');
tz_def('OASIS_MAX_NOW', 'أكبر عدد الآن');
tz_def('OASIS_MAX_NEXT', 'أكبر عدد في المستوى');
tz_def('OASIS_UNIT_SING', 'واحة');
tz_def('OASIS_IN_SQUARE', 'ضمن المربع');
tz_def('REVIVE', 'إحياء');
...
tz_def('HEROSMANSION_DESC', 'في قصر الأبطال يمكنك إحتلال الواحات القريبة منك كلما زاد مستوى قصر الأبطال زاد عدد الواحات الممكن إحتلالها. وفي قصر الأبطال يمكنك أيضا إنعاش البطل بخبرته او تدريب بطل جديد');
...
tz_def('COSTS_UPGRADING_LEVEL', 'تكلفة الارتقاء إلى المستوى');
...
tz_def('UPGRADE_LEVEL', 'الارتقاء إلى المستوى');
...
tz_def('DELETE_ACCOUNT2', 'يمكنك ان تحذف حسابك هنا. فمتى تبدأ حذف حسابك يبقى حسابك نشطا لمدة يوم واحد. لديك الفرصة لإلغاء الحذف في غضون ال 24 ساعة الاولى.');
tz_def('PREF_VACATION_HUB_HINT', 'الاجازة هي تعطيل كامل للحساب لمدة ثلاث أيام ( تتعطل آليا عند انتهاء المدة او يدويا باي وقت ان ازلتها بنفسك ) ويكلف تفعيلها 5000 ذهبه ، والغاؤها قبل موعدها يكلف ايضا 5000 ذهبه . يمكنك تفعيل الاجازة مجدداً بعد انتهاء الاولى باسبوع.');
tz_def('PREF_VACATION_TITLE', 'الإجازة');
tz_def('PREF_WEB_NOTIF_BODY', 'تم تفعيل إشعارات الويب على هذا المتصفح.');
tz_def('PREF_WEB_NOTIF_TITLE', 'إشعارات الويب');
tz_def('YES', 'نعم');
tz_def('NO', 'لا');
...
tz_def('PREF_MOBILE_MODE', 'وضع الجوال');
tz_def('PREF_OPTIONS', 'خيارات');
tz_def('PREF_WEB_NOTIFICATIONS', 'إشعارات الويب');
tz_def('PREF_LINKED_ACCOUNTS', 'الحسابات المرتبطة');
tz_def('PREF_MANAGE', 'إدارة');
tz_def('PREF_OPEN_OPTIONS', 'فتح خيارات العرض');
tz_def('PREF_NOTIFICATION_SETTINGS', 'إعدادات الإشعارات');
tz_def('PREF_LINKED_ACCOUNTS_SETTINGS', 'إعدادات الحسابات المرتبطة');
tz_def('PREF_MOBILE_AUTO', 'تلقائي');
tz_def('PREF_MOBILE_DESKTOP', 'الكمبيوتر');
tz_def('PREF_MOBILE_PHONE', 'الجوال');
tz_def('PREF_TIMER_REFRESH', 'التحديث عند انتهاء الوقت');
tz_def('PREF_INVERT_COLORS', 'عكس الألوان');
tz_def('PREF_STATS_FORMAT', 'شكل الإحصائيات');
tz_def('PREF_STATS_AUTO', 'تلقائي');
tz_def('PREF_STATS_CLASSIC', 'كلاسيكي');
tz_def('PREF_STATS_COMPACT', 'مختصر');
tz_def('PREF_NIGHT_MODE', 'الوضع الليلي');
tz_def('PREF_NIGHT_AUTO', 'تلقائي');
tz_def('PREF_LIGHT_MODE', 'الوضع الفاتح');
tz_def('PREF_DARK_MODE', 'الوضع الليلي');
tz_def('PREF_NAME_RESERVATION', 'حجز الأسماء');
tz_def('PREF_HERO_NAME_MANAGE', 'إدارة اسم البطل');
tz_def('PREF_HERO_COUNT', 'الأبطال المسجلون');
tz_def('PREF_YES', 'نعم');
tz_def('PREF_NO', 'لا');
tz_def('PREF_SECTION_PREMIUM', 'خدمات مميزة');
tz_def('PREF_SECTION_AUTH', 'التفويض');
tz_def('PREF_SECTION_OPTIONS', 'خيارات');
tz_def('PREF_SECTION_NOTIF', 'الإشعارات');
tz_def('PREF_CHANGE_NAME', 'تغيير الاسم');
tz_def('PREF_CHANGE_NAME_HINT', 'تغيير اسم البطل أو القرية');
tz_def('PREF_SITTER_NOTIF', 'إشعارات الوكيل');
tz_def('PREF_SITTER_NOTIF_HINT', 'تنبيهات عند دخول الوكيل لحسابك');
tz_def('PREF_STOP_AUTO_UPDATE', 'إيقاف التحديث التلقائي');
tz_def('PREF_BLOCK_MESSAGES', 'حظر إرسال الرسائل');
tz_def('PREF_REDIRECT_BEHAVIOR', 'سلوك التوجيه بعد الترقية');
tz_def('PREF_MAP_STYLE', 'نمط الخريطة');
tz_def('PREF_MAP_STYLE_HINT', 'خريطة كبيرة أو عادية');
tz_def('PREF_STATUS_BLOCKED', 'محظورة');
tz_def('PREF_NAME_SLOT', '%d/%d');
tz_def('PREF_STAY_IN_BUILDING', 'البقاء في المبنى');
...
//ANMELDEN.php (تكملة)
tz_def('BEFORE_REGISTER', 'قبل التسجيل في '.SERVER_NAME.' يمكنك قراءة <a href="anleitung.php">الدليل</a> لمعرفة ميز وعيوب القبائل');
tz_def('REG_EMAIL_LABEL', 'البريد الالكتروني');
tz_def('REG_PW_LABEL', 'كلمة السر');
tz_def('REG_CHOOSE_TRIBE', 'إختر قبيلة');
tz_def('REG_START_POS', 'موقع البداية');
tz_def('REG_WORLD_STARTED', 'بدأ هذا العالم منذ');
tz_def('REG_HOUR', 'ساعة');
tz_def('REG_TRIBE_ROMANS', 'الرومان');
tz_def('REG_TRIBE_TEUTONS', 'الجرمان');
tz_def('REG_TRIBE_GAULS', 'الأغريق');
tz_def('REG_POS_RANDOM', 'عشوائي');
tz_def('REG_POS_NW', 'شمال غربي');
tz_def('REG_POS_NE', 'شمال شرقي');
tz_def('REG_POS_SW', 'جنوب غربي');
tz_def('REG_POS_SE', 'جنوب شرقي');
tz_def('REG_MAP_FULL', '<li>لا يوجد مكان فارغ في موقع البداية المختار. اختر «عشوائي» أو جرّب ربع الخريطة الآخر.</li>');
tz_def('REG_LIMIT_IP', '<li>تم الوصول إلى حد التسجيل من هذا الجهاز أو عنوان IP. جرّب لاحقاً أو استخدم حساباً موجوداً.</li>');
tz_def('REG_PLUS_GOLD_TITLE', 'الذهب');
tz_def('REG_PLUS_CHARGE', 'شحن الذهب');
tz_def('REG_PLUS_FEATURES', 'الميزات');
tz_def('REG_PLUS_BALANCE', 'الرصيد');
tz_def('REG_PLUS_EARN', 'اكسب ذهب');
tz_def('REG_PLUS_TRANSFER', 'تحويل الذهب');
tz_def('REG_PLUS_HISTORY', 'عمليات سابقة');
tz_def('TZ_GK_PLUS_TRANSFER_TITLE', 'تحويل الذهب من لاعب إلى آخر');
tz_def('TZ_GK_PLUS_TRANSFER_INTRO', 'يمكنك تحويل الذهب المشترى أو ذهب المعجزة فقط. يمكنك تحويل :');
tz_def('TZ_GK_PLUS_TRANSFER_PLAYER', 'اسم اللاعب :');
tz_def('TZ_GK_PLUS_TRANSFER_AMOUNT', 'عدد الذهب :');
tz_def('TZ_GK_PLUS_TRANSFER_WORLD', 'العالم :');
tz_def('TZ_GK_PLUS_TRANSFER_PASSWORD', 'كلمة السر للتأكيد:');
tz_def('TZ_GK_PLUS_TRANSFER_SUBMIT', 'أرسل');
...
tz_def('LOGIN_PAGE_TITLE', 'الدخول الى اللعبة');
tz_def('LOGIN_HELP_TEXT', 'يمكنك الدخول بواسطة الاسم او البريد الالكتروني و إذا تعدد مستخدموا هذا الحاسب يجب أن تضغط على تسجيل الخروج في كل مرة تنتهي من اللعب وذلك حفاظاً على بياناتك.');
tz_def('LOGIN_USER_OR_EMAIL', 'الاسم المستعار أو البريد:');
tz_def('LOGIN_SUBMIT', 'تسجيل الدخول');
tz_def('LOGIN_SHOW_PASSWORD', 'إظهار / إخفاء كلمة المرور');
tz_def('LOGIN_MENU_ENTER', 'دخول');
tz_def('LOGIN_MENU_REG', 'سجل الآن');
tz_def('LOGIN_QUICK_GUIDE', 'الدليل السريع');
tz_def('LOGIN_GAME_GUIDE', 'شرح اللعبة');
tz_def('LOGIN_NOW', 'الآن');
tz_def('LOGIN_WORLD_TAG', '');
tz_def('LOGIN_STAT_PLAYERS', 'عدد اللاعبين :');
tz_def('LOGIN_STAT_ACTIVE', 'اللاعبون النشطون :');
tz_def('LOGIN_STAT_ONLINE', 'اللاعبون الموجودون حاليا :');
tz_def('LOGIN_STAT_WORLD_AGE', 'بدأ العالم');
tz_def('LOGIN_STAT_ARTEFACTS_SINCE', 'مضى على نزول التحف');
tz_def('LOGIN_STAT_ARTEFACTS_LEFT', 'بقي على نزول التحف');
tz_def('LOGIN_STAT_NATARS_LEFT', 'بقي على ظهور التتار');
tz_def('LOGIN_STAT_NATARS_SINCE', 'مضى على ظهور التتار');
tz_def('TZ_STAT_NATARS_LEFT', 'بقي على نزول التتار');
tz_def('TZ_STAT_NATARS_SINCE', 'مضى على نزول التتار');
tz_def('TZ_STAT_NATARS_TAB', 'التتار');
tz_def('TZ_STAT_WORLD_TIMES', 'مواعيد العالم');
tz_def('TZ_STAT_WORLD_INFO', 'معلومات العالم');
tz_def('TZ_STAT_WORLD_STARTED', 'بدأ العالم منذ');
tz_def('TZ_STAT_ARTIFACTS_APPEAR', 'ظهور التحف');
tz_def('TZ_STAT_NATARS_APPEAR', 'ظهور التتار');
tz_def('TZ_STAT_HOURS_SUFFIX', 'ساعة');
tz_def('TZ_STAT_SINCE_PREFIX', 'منذ');
tz_def('TZ_STAT_LEFT_PREFIX', 'بقي');
tz_def('TZ_STAT_GAME_SPEED', 'سرعة اللعبة');
tz_def('TZ_STAT_TROOP_SPEED', 'سرعة الجيش');
tz_def('TZ_STAT_PROD_SPEED', 'سرعة الإنتاج');
tz_def('TZ_STAT_STORAGE_SIZE', 'حجم المخازن');
tz_def('TZ_STAT_PLAYER_COUNT', 'عدد اللاعبين');
tz_def('TZ_STAT_ONLINE_NOW', 'المتواجدون الآن');
tz_def('TZ_STAT_NATARS_SPAWN_DAY', 'وقت ظهور التتار منذ بداية العالم باليوم');
tz_def('TZ_STAT_BEGINNER_PROT_H', 'حماية المبتدئين بالساعة');
tz_def('TZ_STAT_TRIBE_PLAYERS', 'اللاعبون');
tz_def('TZ_STAT_TRIBE_PERCENT', 'النسبة');
tz_def('TZ_STAT_TRIBE_ARABS', 'العرب');
tz_def('TZ_STAT_WORLD_NEWS', 'أهم أخبار العالم');
tz_def('TZ_STAT_NEWS_COL', 'الخبر');
tz_def('TZ_STAT_NEWS_SINCE', 'منذ');
tz_def('TZ_STAT_NEWS_ATTACK', 'قام اللاعب %s بالهجوم على %s ومسح دفاعاته بقتل %s جندي');
tz_def('TZ_STAT_NEWS_FOOTNOTE', '* الهجمات ذات نقاط القتل القليله لاتظهر في هذه القائمة');
tz_def('TZ_STAT_NEWS_EMPTY', 'لا توجد أخبار هجمات كبيرة حالياً.');
tz_def('TZ_STAT_TOP10_TAB', 'قائمة 10');
tz_def('TZ_STAT_UNDEFEATED_TITLE', 'لم يُهزموا في الدفاع منذ بداية العالم');
tz_def('TZ_STAT_UNDEFEATED_SINCE', 'منذ');
tz_def('TZ_STAT_UNDEFEATED_DAILY', 'كسب كل يوم');
tz_def('TZ_STAT_UNDEFEATED_BREAK', 'كسر');
tz_def('TZ_STAT_UNDEFEATED_ATTACK', 'إهجم');
tz_def('TZ_STAT_TOP10_ATT_DAY', 'مهاجموا اليوم');
tz_def('TZ_STAT_TOP10_DEF_DAY', 'مدافعو اليوم');
tz_def('TZ_STAT_TOP10_DEV_DAY', 'مطورو اليوم');
tz_def('TZ_STAT_TOP10_ROB_DAY', 'ناهبو اليوم');
tz_def('TZ_STAT_TOP10_MEDAL_AFTER', 'بعد');
tz_def('TZ_STAT_TOP10_MEDAL_HOURS', 'ساعة ستوزع الأوسمة');
tz_def('TZ_STAT_TOP10_MEDAL_EARN', 'في نهاية هذه التوزيعة ستكسب');
tz_def('TZ_STAT_TOP10_MEDAL_DAILY', 'توزيع الأوسمة يتم كل يوم');
tz_def('TZ_STAT_TOP10_MEDAL_EVERY', 'توزيع الأوسمة يتم كل %s');
tz_def('LOGIN_DAY_ZERO', 'اليوم');
tz_def('TZ_DAY_TODAY', 'اليوم');
tz_def('TZ_DAY_YESTERDAY', 'أمس');
tz_def('LOGIN_DAY_ONE', 'يوم');
tz_def('LOGIN_DAY_TWO', 'يومان');
tz_def('LOGIN_DAYS_FEW', 'أيام');
...
tz_def('TZ_MSG_INBOX_TAB', 'الوارد');
tz_def('TZ_MSG_SENT_TAB', 'الصادر');
tz_def('TZ_MSG_WRITE_TAB', 'أكتب');
tz_def('TZ_MSG_CHAT_TAB', 'الشات');
tz_def('TZ_MSG_NOTES_TAB', 'دفتر الملاحظات');
tz_def('TZ_MSG_IGNORED_TAB', 'المتجاهلون');
tz_def('TZ_MSG_INBOX_TITLE', 'الرسائل الواردة');
tz_def('TZ_MSG_SENT_TITLE', 'الرسائل الصادرة');
tz_def('TZ_MSG_WRITE_TITLE', 'كتابة رسالة');
tz_def('TZ_MSG_NOTES_TITLE', 'دفتر الملاحظات');
tz_def('TZ_MSG_ARCHIVE_TITLE', 'الأرشيف');
tz_def('TZ_MSG_MARK_ALL_READ_OR', 'أو اجعلها مقروءة');
tz_def('TZ_MSG_MARK_ALL_READ_OR_PREFIX', 'أو');
tz_def('TZ_MSG_MARK_ALL_READ_LINK', 'اجعلها مقروءة');
tz_def('TZ_PROF_TAB_OVERVIEW', 'نظرة عامة');
tz_def('TZ_PROF_TAB_ABOUT', 'من نحن');
tz_def('TZ_PROF_TAB_MEMBERSHIP', 'العضوية');
tz_def('TZ_PROF_TAB_OPTIONS', 'خيارات');
tz_def('TZ_PROF_TAB_ACCOUNTS', 'الحسابات');
tz_def('TZ_PROF_TAB_NAME_RESERVE', 'حجز الاسم');
tz_def('TZ_PROF_EDIT_INFO', 'تعديل معلوماتك الشخصية');
tz_def('TZ_PROF_RESOURCES_COL', 'موارد');
tz_def('TZ_PROF_TROOPS_COL', 'قوات');
tz_def('TZ_PROF_DISTANCE_COL', 'مسافة');
tz_def('TZ_PROF_POP_COL', 'سكان');
tz_def('TZ_PROF_EDIT_VILLAGES', 'تعديل اسماء القرى');
tz_def('TZ_PROF_COUNTRY', 'البلد');
tz_def('TZ_PROF_CAP_SHORT', 'عاصمة');
tz_def('TZ_PROF_MEDAL_COL', 'الوسام');
tz_def('TZ_PROF_DOVE_PEACE', 'حمامة السلام');
tz_def('TZ_PROF_TRIBE_MEDAL', 'القبيلة');
tz_def('TZ_GK_MEMBERSHIP_SETTINGS', 'تغيير إعدادات العضوية');
tz_def('TZ_GK_MEMBERSHIP_CHANGE_TYPE', 'نوع التغيير');
tz_def('TZ_GK_MEMBERSHIP_NEW_VALUE', 'القيمة الجديدة');
tz_def('TZ_GK_MEMBERSHIP_NEW_PASSWORD', 'كلمة السر الجديدة');
tz_def('TZ_GK_MEMBERSHIP_NEW_EMAIL', 'البريد الإلكتروني الجديد');
tz_def('TZ_GK_MEMBERSHIP_EMAIL_NOTE', 'عند تغيير البريد الإلكتروني يجب تفعيل الحساب من الرابط المرسل إلى بريدك الجديد. عند التفعيل تحصل على');
tz_def('TZ_GK_MEMBERSHIP_AGENCY', 'وكالة العضوية');
tz_def('TZ_GK_MEMBERSHIP_SITTER_1', 'الوكيل الأول');
tz_def('TZ_GK_MEMBERSHIP_SITTER_2', 'الوكيل الثاني');
tz_def('TZ_GK_MEMBERSHIP_SITTER_ARMY', 'الوكيل يمكنه التحكم بالجيش');
tz_def('TZ_GK_MEMBERSHIP_SITTER_GOLD', 'الوكيل يمكنه التحكم بالذهب');
tz_def('TZ_GK_MEMBERSHIP_SITTER_MSG', 'الوكيل يمكنه قراءة الرسائل');
tz_def('TZ_GK_MEMBERSHIP_SITTER_REP', 'الوكيل يمكنه قراءة التقارير');
tz_def('TZ_GK_MEMBERSHIP_YOU_SITTER', 'أنت وكيل على');
tz_def('TZ_GK_MEMBERSHIP_OTHER', 'إعدادات أخرى');
tz_def('TZ_GK_MEMBERSHIP_NO_SEND_REP', 'منع تقارير إرسال الموارد');
tz_def('TZ_GK_MEMBERSHIP_NO_RECV_REP', 'منع تقارير إستقبال الموارد');
tz_def('TZ_GK_MEMBERSHIP_COMMANDS', 'أوامر العضوية');
tz_def('TZ_GK_MEMBERSHIP_DELETE', 'حذف العضوية');
tz_def('TZ_GK_MEMBERSHIP_VACATION', 'تفعيل الإجازة لمدة 48:00:00 ساعة');
tz_def('TZ_GK_MEMBERSHIP_SAVE_PW', 'كلمة المرور للحفظ');
tz_def('TZ_GK_MEMBERSHIP_SAVE_PW_REQUIRED', 'أدخل كلمة المرور الحالية للحفظ.');
tz_def('TZ_GK_QUICK_LINKS', 'روابط سريعة');
tz_def('TZ_GK_LINK_TITLE', 'العنوان');
tz_def('TZ_GK_LINK_TARGET', 'الهدف');
tz_def('TZ_GK_NAME_RESERVE_NEED_WW', 'يجب ان تكون قد فزت مسبقا بمعجزة العالم بهذا البريد الالكتروني ليمكنك حجز الاسم');
tz_def('TZ_GK_OPT_COL_OPTION', 'الخيار');
tz_def('TZ_GK_OPT_COL_VALUES', 'القيم');
tz_def('TZ_GK_OPT_TIMER_NO_EXAMPLE', 'لا (مثال:00:00:10)');
tz_def('TZ_GK_OPT_STATS_SHAPE', 'شكل الاحصائيات');
tz_def('TZ_GK_OPT_SAVED', 'تم حفظ الخيارات.');

tz_def('TZ_GK_ACC_INTRO', 'العضويات التابعة لك هي عضويات تملكها أنت وليست عضويات لاعبين آخرين ، يسمح للمستخدم بامتلاك %d عضويات كحد أقصى، من هنا يمكنك تسجيل عضوياتك وذلك لتستثنى من منع إرسال الموارد عبر العضويات على نفس الانترنت');
tz_def('TZ_GK_ACC_ADD_TITLE', 'إضافة عضوية تابعة');
tz_def('TZ_GK_ACC_OTHER_NAME', 'اسم العضوية الأخرى');
tz_def('TZ_GK_ACC_PASSWORD', 'كلمة المرور');
tz_def('TZ_GK_ACC_LINK_BTN', 'ربط العضوية');
tz_def('TZ_GK_ACC_WARNING', '* لايمكن إلغاء أو تغيير العضويات التابعة لك ، لذلك إحرص على اختيار العضويات بشكل صحيح');
tz_def('TZ_GK_ACC_ERR_PASSWORD', 'كلمة المرور غير صحيحة.');
tz_def('TZ_GK_ACC_SUCCESS', 'تم ربط العضوية بنجاح.');
tz_def('TZ_RPT_ATTACK_TAB', 'الهجوم');
tz_def('TZ_RPT_DEFENSE', 'الدفاع');
tz_def('TZ_RPT_SCOUT_TAB', 'التجسس');
tz_def('TZ_RPT_MISSION', 'مهمة');
tz_def('TZ_MARK_ALL_READ', 'اجعلها مقروءة');
tz_def('TZ_RPT_MARK_ALL_READ_OR_PREFIX', 'أو');
tz_def('TZ_RPT_MARK_ALL_READ_LINK', 'إجعله مقروءة');
...
tz_def('TZ_VACATION_REAL_GOLD_REQUIRED', 'تفعيل وضع الإجازة يتطلب رصيد ذهب حقيقي تم شراؤه فعليًا. الذهب الممنوح من الإدارة لا يمكن استخدامه لتفعيل وضع الإجازة.');
tz_def('ADM_VACATION_TEST_MODE', 'وضع اختبار الإجازة (بذهب الأدمن)');
tz_def('ADM_VACATION_TEST_MODE_DESC', 'للاختبار فقط. عند التفعيل (ON)، يُقبل الذهب الذي يمنحه الأدمن للاعب (Add Gold) أيضًا لتفعيل وضع الإجازة، حتى يتمكن المختبِرون من التجربة دون شراء حقيقي. عند الإيقاف (OFF) - وهو الوضع الموصى به في السيرفر الحقيقي - يُقبل فقط رصيد ذهب مُشترى فعليًا. هذا لا يغيّر تصنيف الذهب المدفوع/المجاني في أي مكان آخر من اللعبة.');
tz_def('ADM_TESTING_ONLY_BADGE', 'للاختبار فقط');
tz_def('ADM_NOT_PRESENT_IN_CONFIG_PHP_YET_SAVING_THIS_FO', 'غير موجود في config.php بعد - حفظ هذا النموذج سيضيفه.');
...
tz_def('TZ_THE_LARGEST_PLAYERS', 'أكبر اللاعبون');
...
tz_def('TZ_THE_LARGEST_HEROES', 'أكبر الأبطال');
tz_def('TZ_HERO_NAME_COL', 'اسم البطل');
tz_def('TZ_HERO_OFF_EFF', 'كفاءة هجوم');
tz_def('TZ_HERO_DEF_EFF', 'كفاءة دفاع');
tz_def('TZ_HERO_IND_STRENGTH', 'قوة فردية');
tz_def('TZ_HERO_LIFE', 'الحياة');
tz_def('TZ_HERO_ALIVE', 'حي');
tz_def('TZ_HERO_DEAD', 'ميت');
...
tz_def('FS_PLAYER_INTRO', 'لا يمكنك ربط حسابك (حساباتك) البديلة في هذا العالم حتى لا تتمكن من نهبها بحرية للتدريب أو مشاركة الموارد، دون الحد المعتاد لنهب المخبأ.');
...
tz_def('ADM_XG_TYPE_POINTS_AWARDED', 'تم منح نقاط');
tz_def('ADM_XG_TYPE_GOLD_CONVERTED', 'تم تحويل الذهب');
tz_def('ADM_XG_TYPE_ADMIN_ADJUST', 'تعديل بواسطة الأدمن');
tz_def('ADM_XG_SOURCE_DAILY_LOGIN', 'تسجيل الدخول اليومي');
tz_def('ADM_XG_SOURCE_WEBHOOK', 'نشاط X-Tatar.com');
tz_def('ADM_XG_SOURCE_ADMIN', 'تعديل بواسطة الأدمن');
tz_def('ADM_XG_SOURCE_OTHER', 'أخرى');
tz_def('ADM_XG_NOTE_WEBHOOK', 'Webhook نشاط X-Tatar.com');
tz_def('ADM_XG_NOTE_COLUMN', 'الملاحظة');
...
tz_def('ADMIN_PANEL_TITLE', 'لوحة الأدمن');
tz_def('ADMIN_LOGGED', 'مسجّل الدخول');
tz_def('ADMIN_NOT_LOGGED_IN', 'غير مسجّل الدخول');
tz_def('ADMIN_ROLE_ADMIN', 'أدمن');
tz_def('ADMIN_ROLE_USER', 'مستخدم');
tz_def('ADMIN_PUNISHMENTS', 'العقوبات (كتم / السوق / الجيش)');
tz_def('ADMIN_PUNISHMENTS_SHORT', 'العقوبات');
tz_def('ADMIN_CHAT_MODERATION', 'إدارة الدردشة');
tz_def('ADMIN_EDIT_TROOPS_TITLE', 'تعديل القوات');
tz_def('ADMIN_DELETE_ALLIANCE_CONFIRM', 'هل تريد حذف التحالف؟');
tz_def('ADMIN_T4_HERO_CONTROLS_TITLE', 'أدوات تحكم بطل T4');
tz_def('ADMIN_T4_RUNNING_ADVENTURE_NOTE', 'يصل في الموعد المحدد (لا تحذف المغامرات الجارية).');
tz_def('ADMIN_SERVER_GRAPHIC_PACK', 'حزمة رسومات السيرفر');
tz_def('ADMIN_DATE', 'التاريخ');
tz_def('ADMIN_TIME', 'الوقت');
tz_def('ADMIN_TRUE', 'صحيح');
tz_def('ADMIN_FALSE', 'خطأ');
tz_def('ADMIN_ENABLED', 'مفعّل');
tz_def('ADMIN_DISABLED', 'معطّل');
tz_def('ADMIN_LIMIT_MAILBOX_DISABLED', 'حد صندوق الرسائل معطّل');
tz_def('ADMIN_FEATURE_FLAGS_TITLE', 'مفاتيح تفعيل الميزات');
tz_def('ADMIN_FEATURE_FLAGS_INTRO', 'أضف أو احذف مفاتيح تشغيل وإيقاف لأي ميزة دون الحاجة إلى تعديل الكود أو ملف config.php. تُطبّق المفاتيح فور تغييرها، ولا تحتاج إلى إعادة تشغيل السيرفر أو إعادة إنشاء الإعدادات. هذه الصفحة منفصلة عن إعدادات السيرفر وبلس والسجلات، التي تتحكم في المفاتيح المدمجة؛ أما هنا فهي للمفاتيح التي تنشئها بنفسك.');
tz_def('ADMIN_ADD_FLAG', 'إضافة مفتاح ميزة جديد');
tz_def('ADMIN_KEY', 'المفتاح');
tz_def('ADMIN_LABEL', 'الاسم');
tz_def('ADMIN_NOTE_OPTIONAL', 'ملاحظة (اختياري)');
tz_def('ADMIN_ENABLED', 'مفعّل');
tz_def('ADMIN_ADD_FLAG_BUTTON', 'إضافة المفتاح');
tz_def('ADMIN_NO_FEATURE_FLAGS', 'لا توجد مفاتيح ميزات بعد. أضف مفتاحًا من الأعلى.');
tz_def('ADMIN_STATUS', 'الحالة');
tz_def('ADMIN_UPDATED', 'آخر تحديث');
tz_def('ADMIN_ON', 'مفعّل');
tz_def('ADMIN_OFF', 'متوقف');
tz_def('ADMIN_DISABLE', 'تعطيل');
tz_def('ADMIN_ENABLE', 'تفعيل');
tz_def('ADMIN_DELETE_FLAG_CONFIRM', 'هل تريد حذف هذا المفتاح؟ أي كود يستخدمه سيعود إلى قيمته الافتراضية.');
tz_def('ADMIN_FLAG_NOTE_PLACEHOLDER', 'ما الذي يتحكم فيه هذا المفتاح');
tz_def('ADMIN_FLAG_HINT', 'تُحوّل أسماء المفاتيح إلى أحرف صغيرة وتُطبّع لتتكون من الحروف والأرقام والشرطة السفلية. افحص المفتاح في الكود باستخدام FeatureFlags::isEnabled(\'your_key\').');
tz_def('ADMIN_ACCESS_DENIED', 'الوصول مرفوض.');
tz_def('ADMIN_FEATURE_FLAGS', 'مفاتيح تفعيل الميزات');
tz_def('ADMIN_ADD_REMOVE_FLAGS_INTRO', 'أضف أو احذف مفاتيح تشغيل وإيقاف لأي ميزة دون الحاجة إلى تعديل الكود أو ملف config.php. تُطبّق المفاتيح فور تغييرها، ولا تحتاج إلى إعادة تشغيل السيرفر أو إعادة إنشاء الإعدادات. هذه الصفحة منفصلة عن إعدادات السيرفر وبلس والسجلات، التي تتحكم في المفاتيح المدمجة؛ أما هنا فهي للمفاتيح التي تنشئها بنفسك.');
tz_def('ADMIN_GRAPHIC_PACK_INFO', 'حزمة الرسوميات التي يراها كل لاعب افتراضيًا. تُقرأ الحزم من مجلد gpack/، ويُعتبر المجلد حزمة إذا احتوى على novaterra.css. تغييرها يبدّل مظهر السيرفر بالكامل.');
tz_def('ADMIN_DELETE', 'حذف');
tz_def('ADMIN_MESSAGE_SENT', 'تم إرسال الرسالة بنجاح');
tz_def('ADMIN_NORMAL_USER', 'مستخدم عادي');
tz_def('ADMIN_NO_SITTER', 'لا يوجد وكيل');
tz_def('ADMIN_EDIT_FAILED', 'فشل التعديل.');
tz_def('ADMIN_AVERAGE_RANK', 'متوسط الترتيب');
tz_def('ADMIN_DELETE_ALL', 'حذف الكل');
tz_def('ADMIN_PLAYERS_REPORTS', 'تقارير اللاعبين');
tz_def('ADMIN_FOUND_PLAYERS', 'اللاعبون الذين تم العثور عليهم');
tz_def('ADMIN_EDIT', 'تعديل');
tz_def('ADMIN_NOT_FOUND', 'غير موجود');
tz_def('ADMIN_BACK', 'رجوع');
tz_def('ADMIN_EDIT_ACCESS', 'تعديل الصلاحية');
tz_def('ADMIN_ACCESS_CHANGED', 'تم تغيير صلاحيات اللاعبين');
tz_def('ADMIN_CANNOT_CHANGE_OWN_ACCESS', 'لا يمكنك تغيير صلاحيتك الخاصة');
tz_def('ADMIN_PLAYER_NOT_FOUND', 'اللاعب غير موجود.');
tz_def('ADMIN_GO_BACK', 'العودة');
tz_def('ADMIN_FOUND_ALLIANCES', 'التحالفات التي تم العثور عليها');
tz_def('ADMIN_FOUND_EMAILS', 'عناوين البريد التي تم العثور عليها');
tz_def('ADMIN_FOUND_IPS', 'عناوين IP التي تم العثور عليها');
tz_def('ADMIN_NO_LOGIN_RECORDS', 'لا توجد سجلات دخول.');
tz_def('ADMIN_FOUND', 'تم العثور على');
tz_def('ADMIN_INSTRUCTIONS', 'التعليمات');
tz_def('ADMIN_CONTACT_SUPPORT', 'هل جرّبت كل شيء دون فائدة؟ اكتب ردًا على هذه الرسالة للتواصل مع الدعم');
tz_def('ADMIN_ACTIVE_SERVER_FEATURES', 'ميزات السيرفر النشطة');
tz_def('ADMIN_NOW', 'الآن');
tz_def('ADMIN_LAST_ACTION', 'آخر إجراء');
tz_def('ADMIN_JUST_NOW', 'الآن للتو');
tz_def('ADMIN_MIN_AGO', 'دقيقة مضت');
tz_def('ADMIN_ALL_FILTER', 'الكل');
tz_def('ADMIN_ATTACKS', 'الهجمات');
tz_def('ADMIN_DEFENCES', 'الدفاعات');
tz_def('ADMIN_SCOUTS', 'الاستطلاع');
tz_def('ADMIN_TRADES', 'التجارة');
tz_def('ADMIN_PLAYERS_MESSAGES', 'رسائل اللاعبين');
tz_def('ADMIN_SYSTEM', 'النظام');
tz_def('ADMIN_NO_SUBJECT', '(بلا موضوع)');
tz_def('ADMIN_MAP_TITLE', 'الخريطة');
tz_def('ADMIN_PLAYER', 'اللاعب');
tz_def('ADMIN_COORD', 'الإحداثيات');
tz_def('ADMIN_POP', 'السكان');
tz_def('ADMIN_ARTIFACT', 'القطعة الأثرية');
tz_def('ADMIN_PROFILE', 'الملف الشخصي');
tz_def('ADMIN_NO_UNACTIVATED_PLAYERS', 'لا يوجد لاعبون غير مفعّلين');
...
tz_def('ADM_PUN_TITLE', 'العقوبات (كتم / السوق / تجميد الجيش)');
tz_def('ADM_PUN_MUTED_NOW', 'مكتوم الآن');
tz_def('ADM_PUN_MARKET_NOW', 'السوق مقيّد الآن');
tz_def('ADM_PUN_ARMY_NOW', 'الجيش مجمّد الآن');
tz_def('ADM_PUN_APPLY_TITLE', 'تطبيق عقوبة');
tz_def('ADM_PUN_USER_ID', 'معرف المستخدم');
tz_def('ADM_PUN_TYPE_MUTE', 'كتم (الدردشة)');
tz_def('ADM_PUN_TYPE_MARKET', 'تقييد السوق');
tz_def('ADM_PUN_TYPE_ARMY', 'تجميد الجيش');
tz_def('ADM_PUN_REASON_PH', 'السبب (مطلوب، يظهر للاعب)');
tz_def('ADM_PUN_APPLY_BTN', 'تطبيق العقوبة');
tz_def('ADM_PUN_LABEL_MUTE', 'كتم الدردشة');
tz_def('ADM_PUN_LABEL_MARKET', 'السوق');
tz_def('ADM_PUN_LABEL_ARMY', 'تجميد الجيش');
tz_def('ADM_PUN_ACTIVE', 'نشط');
tz_def('ADM_PUN_NO_ACTIVE', 'لا يوجد حاليًا عقوبات %s نشطة');
tz_def('ADM_PUN_CONFIRM_LIFT', 'هل تريد رفع هذه العقوبة؟');
tz_def('ADM_PUN_ERR_INVALID_UID', 'معرف مستخدم غير صالح!');
tz_def('ADM_PUN_ERR_SYSTEM_ACC', 'لا يمكنك تقييد حسابات النظام!');
tz_def('ADM_PUN_ERR_INVALID_TYPE', 'نوع العقوبة غير صالح!');
tz_def('ADM_PUN_ERR_REASON_REQUIRED', 'السبب مطلوب!');
tz_def('ADM_PUN_ERR_NO_USER', 'هذا المستخدم غير موجود!');
tz_def('ADM_PUN_SUCCESS_APPLY', 'تم تطبيق عقوبة <b>%s</b> على <b>%s</b>.');
tz_def('ADM_PUN_ERR_APPLY_FAILED', 'تعذّر تطبيق هذه العقوبة!');
tz_def('ADM_PUN_ERR_INVALID_REQUEST', 'طلب غير صالح.');
tz_def('ADM_PUN_LIFTED', 'تم رفع العقوبة.');
tz_def('ADM_PUN_PERMANENT', 'دائم (حتى الرفع)');
tz_def('ADM_BAN_ERR_INVALID_UID', 'معرف مستخدم غير صالح!');
tz_def('ADM_BAN_ERR_SYSTEM_ACC', 'لا يمكنك حظر حسابات النظام!');
tz_def('ADM_BAN_ERR_NO_USER', 'هذا المستخدم غير موجود!');
tz_def('ADM_BAN_ERR_ALREADY_BANNED', 'المستخدم محظور بالفعل!');
tz_def('ADM_BAN_SUCCESS_USER', 'تم حظر المستخدم <b>%s</b> بنجاح!');
tz_def('ADM_BAN_ERR_INVALID_IP', 'عنوان آي بي غير صالح!');
tz_def('ADM_BAN_SUCCESS_IP', 'تم حظر عنوان الآي بي <b>%s</b> بنجاح!');
tz_def('ADM_BAN_ERR_IP_FAILED', 'تعذّر حظر هذا الآي بي!');
tz_def('ADM_BAN_REASON_PUSHING', 'دفع/تلاعب غير مشروع');
tz_def('ADM_BAN_REASON_CHEAT', 'غش');
tz_def('ADM_BAN_REASON_HACK', 'اختراق');
tz_def('ADM_BAN_REASON_BUG', 'استغلال ثغرة');
tz_def('ADM_BAN_REASON_BADNAME', 'اسم مستخدم غير لائق');
tz_def('ADM_BAN_REASON_MULTI', 'تعدد حسابات');
tz_def('ADM_BAN_REASON_SWEARING', 'إساءة/سب');
tz_def('ADM_HOUR_UNIT', 'ساعة');
tz_def('ADM_DAY_UNIT', 'يوم');
tz_def('ADM_NO_ACTIVE_BANS', 'لا يوجد حظر نشط');
tz_def('ADM_NO_ACTIVE_IP_BANS', 'لا يوجد حظر آي بي نشط');
tz_def('ADM_NO_BAN_HISTORY', 'لا يوجد سجل');
tz_def('ADM_CONFIRM_UNBAN_USER', 'هل تريد إلغاء حظر هذا المستخدم؟');
tz_def('ADM_CONFIRM_UNBAN_IP', 'هل تريد إلغاء حظر هذا الآي بي؟');
...
tz_def('ADM_ADMIN_ACCOUNTS_ALWAYS_KEPT', 'حسابات الأدمن تُحفظ دائمًا');
tz_def('ADM_ADMIN_ACCOUNTS_ALWAYS_KEPT_HINT', 'كل حساب بصلاحية أدمن يحتفظ بنفس اسم المستخدم وكلمة المرور، ويُنشأ له قرية جديدة في وسط الخريطة حتى تتمكن من الدخول وإدارة السيرفر بعد إعادة الضبط.');
...
tz_def('ADM_HEATMAP_ACCESS_DENIED', 'غير مسموح بالدخول.');
tz_def('ADM_HEATMAP_MAP_RADIUS', 'نطاق الخريطة');
tz_def('ADM_HEATMAP_GRID', 'شبكة');
tz_def('ADM_HEATMAP_TILES_CELL', 'مربعات/خلية');
tz_def('ADM_HEATMAP_VILLAGE_DENSITY', 'كثافة القرى');
tz_def('ADM_HEATMAP_TRIBE_DENSITY', 'كثافة القبائل');
tz_def('ADM_HEATMAP_INACTIVITY', 'عدم النشاط');
tz_def('ADM_HEATMAP_ATTACKS', 'الهجمات');
tz_def('ADM_HEATMAP_LEGEND', 'المفتاح');
tz_def('ADM_HEATMAP_VILLAGES', 'القرى');
tz_def('ADM_HEATMAP_POPULATION', 'السكان');
tz_def('ADM_HEATMAP_INACTIVE', 'غير نشط');
tz_def('ADM_HEATMAP_DOMINANT', 'المهيمنة');
tz_def('ADM_HEATMAP_CELL_DOMINANT', 'توضح الخلية القبيلة المهيمنة؛ واللون الأفتح يعني قرى أكثر.');
tz_def('ADM_HEATMAP_HIGHER_COUNT', 'الخلايا الأغمق/الأفتح تعني عددًا أكبر في تلك المنطقة.');
...
tz_def('ADM_DEBUG_WARNING', 'تحذير');
tz_def('ADM_DEBUG_NOTICE', 'ملاحظة');
tz_def('ADM_DEBUG_DEPRECATED', 'قديم/مهجور');
tz_def('ADM_DEBUG_FATAL_ERROR', 'خطأ فادح');
tz_def('ADM_DEBUG_PHP_WARNING', 'تحذير PHP');
tz_def('ADM_DEBUG_PHP_NOTICE', 'ملاحظة PHP');
tz_def('ADM_DEBUG_PHP_DEPRECATED', 'تحذير PHP من استخدام قديم');
tz_def('ADM_DEBUG_UNDEFINED_VARIABLE', 'متغير غير معرّف');
tz_def('ADM_DEBUG_UNDEFINED_ARRAY_KEY', 'مفتاح مصفوفة غير معرّف');
tz_def('ADM_DEBUG_UNDEFINED_CONSTANT', 'ثابت غير معرّف');
tz_def('ADM_DEBUG_FAILED_OPEN_STREAM', 'تعذر فتح التدفق');
...
tz_def('ADM_FOUND_VILLAGES', 'القرى المكتشفة');
tz_def('ADM_VILLAGES_FOUND', 'قرى مكتشفة');
tz_def('ADM_NO_VILLAGES_CALLED', 'لا توجد قرى تسمى');

// Admin: massmessage.tpl (added by AR localization phase 2)
tz_def('ADM_IN_GAME_MESSAGE_ALL_USERS', 'رسالة في اللعبة لجميع المستخدمين (المعرّف > 5)');

// Admin: XTatarGold.php (added by AR localization phase 2)
tz_def('ADM_CONSECUTIVE_LOGIN_DAY', 'يوم دخول متتالي');
tz_def('ADM_FIRST_LOGIN_DAY', 'أول دخول في اليوم');

// Admin: editUser.tpl (added by AR localization phase 2)
tz_def('ADM_VILLAGES_LABEL', 'القرى');
tz_def('ADM_GOLD_LABEL', 'الذهب');

// Admin: inactive.tpl (added by AR localization phase 2)
tz_def('ADM_1_3_DAYS', '1-3 أيام');
tz_def('ADM_3_7_DAYS', '3-7 أيام');
tz_def('ADM_7_PLUS_DAYS', '+7 أيام');
tz_def('ADM_NO_USERS_IN_RANGE', 'لا يوجد لاعبون في هذا النطاق');
tz_def('ADM_DAYS_AGO', 'ي');
tz_def('ADM_HOURS_AGO', 'س منذ');
tz_def('ADM_POPULATION_SHORT', 'سكان');
tz_def('ADM_VILLAGES_SHORT', 'قرية');
tz_def('ADM_DELETE_CONFIRM', 'حذف؟');
