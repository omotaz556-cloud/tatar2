...
//COPYRIGHT
tz_def('NOVATERRA_COPYRIGHT', 'Novaterra 100% Clonă Novaterra Open Source.');
...
tz_def('NATAR_COUNTERFORCE', 'Contraatac Tatari');
...
tz_def('Q2_REWARD', 'Recompensa ta:</p>1 zi Novaterra Plus');
...
tz_def('Q9_DESC', 'În Novaterra este mereu ceva de făcut! În timp ce aștepți sosirea armatei uriașe, ar trebui să creștem puțin producția. Extinde toate câmpurile de resurse la nivelul 1.');
...
tz_def('Q10_REWARD', 'Recompensa ta:</p>2 zile Novaterra Plus');
...
tz_def('Q28_DESC', 'Munca în echipă este importantă în Novaterra. Jucătorii care colaborează se organizează în alianțe. Primește o invitație de la o alianță din regiunea ta și alătură-te. Alternativ, poți fonda propria alianță. Pentru asta ai nevoie de ambasadă nivel 3.');
...
tz_def('ONE_DAY_OF_NOVATERRA', '1 zi Novaterra Plus ');
tz_def('TWO_DAYS_OF_NOVATERRA', '2 zile Novaterra Plus ');
...
tz_def('Q25_10_REWARD', 'Recompensa ta:</p>încă 2 zile de Novaterra Plus');
...
tz_def('Q25_28_DESC', 'Lumea Novaterra este periculoasă. Ai construit deja o ascunzătoare pentru protecție împotriva atacatorilor.<br><br>O alianță bună îți va oferi o protecție și mai bună.');
...
tz_def('Q25_29_DESC', 'Munca în echipă este importantă în Novaterra. Jucătorii care colaborează se organizează în alianțe. Primește o invitație de la o alianță din regiunea ta și alătură-te. Alternativ, poți fonda propria alianță. Pentru asta ai nevoie de ambasadă nivel 3.');
...
tz_def('BEFORE_REGISTER', 'Înainte să te înregistrezi ar trebui să citești <a href="/anleitung.php" target="_blank">instrucțiunile</a> Novaterra ro1 pentru a vedea avantajele și dezavantajele specifice ale celor trei triburi.');
...
tz_def('VACATION_DESC', 'Dacă plănuiești să lipsești o perioadă mai lungă și nu vrei să setezi un sitter, poți activa Modul Vacanță. În acest timp contul tău nu va mai produce resurse, PC, cercetări, trupe etc., și nu va primi atacuri, întăriri, raiduri, practic înghețând contul. Ține minte, îngheață doar jocul, nu și timpul. Dacă ești membru Gold Club, acesta va expira în acest timp, iar dacă ai reînnoire automată, aceasta va fi anulată în Modul Vacanță. Reține că trebuie să setezi minim 2 zile de vacanță și NU MAI MULT de 14 zile');
...
tz_def('GREATWAREHOUSE_DESC', 'Hambarul mare are de 3 ori capacitatea unui hambar normal.<br>Această clădire poate fi construită doar în satele cu Minunea Lumii sau cu un artefact tatar special');

tz_def('GREATGRANARY', 'Grânar mare');
tz_def('GREATGRANARY_DESC', 'Grânarul mare are de 3 ori capacitatea unui grânar normal.<br>Această clădire poate fi construită doar în satele cu Minunea Lumii sau cu un artefact tatar special');

tz_def('WONDER', 'Minunea Lumii');
tz_def('WORLD_WONDER', 'Minune Mondială');
tz_def('WONDER_DESC', 'O Minune a Lumii (cunoscută și ca WW) este pe cât de impresionantă sună. Fiecare nivel costă o mulțime de resurse. Este aproape imposibil ca un singur jucător să construiască o WW de unul singur. Motivul este că nu ai nevoie doar de multe resurse, ci și de trupe pentru a-ți proteja prețioasa clădire.<br>Pentru a construi WW ai nevoie de un Plan de Construcție Antic. Îl poți obține atacând un sat tatar cu eroul tău. Trebuie să ai o trezorerie goală nivel 10 și eroul trebuie să supraviețuiască. Cu acel plan și o cantitate extrem de mare de resurse, poți începe minunea lumii.<br>Odată ce ajunge la nivelul 50, vei avea nevoie ca altcineva din alianța ta să aibă un al doilea plan de construcție activ. Nu poți face totul singur.<br>Finalizând o WW la nivelul 100, vei câștiga serverul Novaterra și acesta este sfârșitul lumii de joc.<br>Odată terminată, va apărea un mesaj care arată cine a câștigat și statisticile. Nu mai poți construi, dar poți trimite mesaje până la restartarea serverului');
...
tz_def('NATARS_ACC', 'Cont oficial Tatari');
tz_def('WW_V_M', 'Sat oficial Minunea Lumii');
tz_def('ROMAN_T_M', 'Romanii: Datorită nivelului ridicat de dezvoltare socială și tehnologică, romanii sunt maeștri în construcție și coordonare. De asemenea, trupele lor fac parte din elita din Novaterra. Sunt foarte echilibrați și utili atât în atac cât și în apărare');
...
tz_def('GAUL_T_M', 'Galii: Galii sunt cel mai pașnic trib dintre cele trei din Novaterra. Trupele lor sunt antrenate pentru o apărare excelentă, dar abilitatea lor de atac poate concura cu celelalte două triburi. Galii sunt călăreți înnăscuți și caii lor sunt faimoși pentru viteză. Asta înseamnă că pot lovi inamicul exact unde provoacă cel mai mult rău și rapid');
tz_def('ADMIN_M', 'Administrator oficial server');
tz_def('MH_M', 'Multihunter global oficial server');
tz_def('MH_M2', 'Multihunter este o poziție oficială Novaterra folosită în principal pentru aplicarea regulilor pe server. Toți Multihunterii folosesc contul numit Multihunter cu singurul sat la (0|0). Un Multihunter nu poate juca pe serverul unde este Multihunter, dar poate fi jucător activ pe alte servere');
tz_def('NATURE_M2', 'Trupele naturii sunt animalele care trăiesc în oazele neocupate. Poți folosi simulatorul de luptă pentru a vedea dacă ai suficiente trupe să învingi animalele dintr-o oază pe care vrei să o cucerești, dar amintește-ți că poți doar face raid pe oaze. Ține minte că toate animalele peste Urs pot ucide într-o luptă 1 la 1 trupa de nivel maxim contemporană din Novaterra');
tz_def('TASKMASTER_M', 'Cont Maestru de sarcini');
tz_def('VETERAN_P', 'Jucător veteran');
tz_def('VETERAN_3_M', 'Medalie obținută pentru 3 ani de joc Novaterra');
tz_def('VETERAN_5_M', 'Medalie obținută pentru 5 ani de joc Novaterra');
tz_def('VETERAN_10_M', 'Medalie obținută pentru 10 ani de joc Novaterra');
...
tz_def('TIME_ZONES_DESC', 'Aici poți schimba ora afișată în Novaterra pentru a se potrivi fusului tău orar');
...
tz_def('ARTEFACT', '<h1><b>Artefactele Tatarilor</b></h1>

Zvonuri șoptite răsună prin sate, împărtășind legende spuse doar de cei mai buni povestitori. Se referă la TATARI, cei mai temuți războinici din lumea NOVATERRA. Uciderea lor este visul oricărui erou, scopul oricărui luptător. Nimeni nu știe cum au obținut TATARII o asemenea putere, iar războinicii lor sunt atât de cruzi. Hotărâți să descopere sursa puterii TATARILOR, luptătorii trimit un grup de spioni de elită să-i urmărească. Nu trec multe ore și se întorc cu frică în ochi, balansând teorii fantastice: se pare că puterea tatarilor vine din obiectele misterioase pe care le numesc artefacte, pe care le-au furat de la strămoșii noștri. Încearcă să furi artefactele de la ei, și vei putea controla puterea lor.

<img src="/img/x.gif" class="ArtefactsAnnouncement">

A venit timpul să revendici artefactele. Colaborează cu alianța ta și adu-ți războinicii pentru a obține aceste obiecte râvnite. Totuși, TATARII nu vor renunța la artefacte fără luptă... nici dușmanii tăi. Dacă reușești să recuperezi artefactele și să respingi inamicii, vei putea culege recompensele. Clădirile tale vor deveni incredibil de puternice, iar trupele vor fi mult mai rapide și vor consuma mai puțină hrană. Capturează artefactele, adu glorie imperiului tău și devino o nouă legendă pentru urmașii tăi.
...
Au trecut nenumărate zile de la primele bătălii de pe zidurile satelor blestemate ale înfricoșătorilor Tatari, multe armate atât ale celor liberi cât și ale imperiului tatar s-au luptat și au murit în fața zidurilor multor fortărețe de unde Tatarii au condus cândva toată creația. Acum, cu praful așezat și o relativă liniște instalată, armatele au început să-și numere pierderile și să-și adune morții, mirosul luptei încă plutind în aerul nopții, un miros de măcel de neuitat prin amploare și brutalitate, dar care în curând va fi umbrit de altele. Cele mai mari armate ale celor liberi și ale înfricoșătorilor Tatari se pregăteau pentru încă un asalt reînnoit asupra râvnitelor foste fortărețe ale Imperiului Tatar.
Curând au sosit cercetași povestind despre o priveliște impresionantă și o amintire înfiorătoare, o armată de temut de o mărime de neînchipuit fusese văzută adunându-se la capătul lumii, capitala tatară, o forță atât de mare și de neoprit încât praful marșului lor ar sufoca toată lumina, o forță atât de brutală și nemiloasă încât ar zdrobi orice speranță. Oamenii liberi știau că trebuie să alerge acum, să alerge contra timpului și hoardelor nesfârșite ale Imperiului Tatar pentru a ridica o Minune a Lumii care să readucă pacea și să învingă amenințarea tatară.
...
Zeci de mii de cercetași au cutreierat toată existența căutând în zadar aceste planuri mistice, căutând peste tot mai puțin în temuta Capitală Tatară, dar nu le-au găsit. Astăzi însă, se întorc cu vești bune, se întorc aducând locațiile planurilor, ascunse de armatele Tatarilor în fortărețe secrete construite pentru a fi ascunse de ochii oamenilor.
Acum începe ultima etapă, când cele mai mari armate ale Oamenilor Liberi și ale Tatarilor se vor ciocni în toată lumea pentru soarta a tot ce se află sub ceruri. Acesta este războiul care va răsuna peste eoni, acesta este războiul tău, și aici îți vei grava numele în istorie, aici vei deveni legendă.
...
Cu multe luni în urmă triburile din Novaterra au fost surprinse de întoarcerea neașteptată a Tatarilor. Acest trib din timpuri imemoriale, care îi depășește pe toți în înțelepciune, putere și glorie, era pe cale să tulbure din nou pe cei liberi. Astfel și-au pus toate eforturile în pregătirea unui ultim război împotriva Tatarilor și în înfrângerea lor pentru totdeauna. Mulți s-au gândit la așa-numitele "Minuni ale Lumii", o construcție din multe legende, ca singura soluție. Se spunea că odată finalizată ar face pe oricine invincibil. În cele din urmă făcând constructorii conducători și cuceritori ai întregului Novaterra cunoscut.
...
Astăzi, însă, acest ultim secret va fi dezvăluit. Lipsurile și eforturile trecutului nu vor fi fost în zadar, căci astăzi cercetașii mai multor triburi au obținut cu succes locațiile planurilor de construcție. Bine păzite de Tatari, ele zac ascunse în mai multe oaze ce pot fi găsite în toată Novaterra. Doar cei mai viteji eroi vor putea asigura un astfel de plan și îl vor aduce acasă în siguranță pentru ca construcția să înceapă.

În cele din urmă, vom vedea dacă triburile libere din Novaterra pot din nou să-i depășească pe Tatari și să-i învingă odată pentru totdeauna. Nu fi atât de prost încât să presupui că Tatarii vor pleca fără luptă, totuși!

<img src="/img/x.gif" class="WWBuildingPlansAnnouncement" title="'.PLAN.'" alt="'.PLAN.'">

Pentru a fura un set de Planuri de Construcție de la Tatari, trebuie să se întâmple următoarele:
...
tz_def('CONF_SERV_NATUNITSMULTIPLIER', 'Multiplicator Unități Tatari');
tz_def('CONF_SERV_NATUNITSMULTIPLIER_TOOLTIP', 'Acest parametru este responsabil pentru numărul trupelor tatarilor, pe artefacte și sate WW.');
tz_def('CONF_SERV_NATARS_SPAWN_TIME', 'Apariție Tatari');
tz_def('CONF_SERV_NATARS_SPAWN_TIME_TOOLTIP', 'După câte zile de la startul serverului vor apărea Tatarii și artefactele');
...
tz_def('CONF_SERV_NATARSTAT', 'Arată Tatarii în Statistici');
tz_def('CONF_SERV_NATARSTAT_TOOLTIP', 'Activează (True) sau dezactivează (False) afișarea contului Tatarilor în statistici.');
...
$lang['index'][0][13] = 'Joacă alături și împotriva a mii de alți jucători reali și cucerește lumea Novaterra.';
...
tz_def('TZ_FEATURES_OF_NOVATERRA', 'Caracteristicile Novaterra');
...
tz_def('TZ_RELEASED_BY_NOVATERRA_TEAM', 'Lansat de: echipa Novaterra');
tz_def('TZ_RELEASE_BY_NOVATERRA', '[Lansat de: Novaterra]');
...
tz_def('TZ_ADDITIONALLY_THE_NOVATERRA_TEAM_WILL', 'În plus, echipa Novaterra nu va furniza informații despre interdicții niciunei persoane, în afară de proprietarul contului.');
tz_def('TZ_ADVERTISEMENT_OF_ANY_KIND_THAT_HAS', 'Orice fel de publicitate care nu a fost permisă de echipa Novaterra este interzisă.');
...
tz_def('TZ_BLACKMAILING_PLAYERS_IN_A_WAY_THAT', 'Șantajarea jucătorilor într-un mod care încalcă oricare dintre regulile Novaterra conform Termenilor și Condițiilor Generale.');
...
tz_def('TZ_HERE_YOU_CAN_CHANGE_NOVATERRA_S_DISP', 'Aici poți schimba ora afișată de Novaterra pentru a se potrivi cu fusul tău orar.');
...
tz_def('TZ_INCITING_MANIPULATING_ENCOURAGING', 'Incitarea, manipularea, încurajarea, asistarea sau conspirarea cu alții pentru a încălca oricare dintre regulile Novaterra este interzisă. Aceste reguli se aplică fără excepție jucătorilor care își vor șterge conturile sau le șterg în prezent.');
...
tz_def('TZ_IN_ORDER_TO_PLAY_NOVATERRA_YOU_NEED', 'Pentru a juca Novaterra ai nevoie de o adresă de e-mail validă la care poate fi trimis codul de activare. Există cazuri excepționale în care acest e-mail ar putea să nu ajungă.');
...
tz_def('TZ_IN_ORDER_TO_SWITCH_AN_ACCOUNT_WITH', 'Pentru a schimba un cont cu o altă persoană pe aceeași lume de joc, ambele persoane trebuie să trimită un e-mail la admin@novaterra.com de la adresa de e-mail înregistrată în prezent pentru cont. E-mailul trebuie să conțină următoarele informații:');
tz_def('TZ_IN_THE_BEGINNING_YOUR_SMALL_VILLAG', 'La început, micul tău sat va avea doar o singură clădire.');
tz_def('TZ_IN_NOVATERRA_YOU_ARE_NOT_ALONE_YOU_I', 'În Novaterra nu ești singur; interacționezi cu mii de alți jucători în lumea Novaterra.');
...
tz_def('TZ_NOW_YOU_KNOW_EVERYTHING_IMPORTANT', 'Acum știi tot ce este important despre Novaterra. După înregistrare poți începe să joci!');
...
tz_def('TZ_RESOURCES_BUILDINGS_VILLAGES_OR_TR', 'Resursele, clădirile, satele sau trupele pierdute în perioada de suspendare nu contează ca pedeapsă și nu vor fi înlocuite de echipa Novaterra. Niciun jucător nu are dreptul să ceară plată sau înlocuire pentru timpul de Plus/Aur pierdut din cauza suspendării.');
...
tz_def('TZ_THERE_ARE_FOUR_DIFFERENT_TYPES_OF', 'Există patru tipuri diferite de resurse în Novaterra: lemn, lut, fier și grâne.');
tz_def('TZ_THERE_IS_NO_COMPENSATION_FOR_DAMAG', 'Nu există compensație pentru daunele provocate de un înlocuitor. Proprietarii de cont sunt pe deplin responsabili pentru acțiunile efectuate de înlocuitorii aleși ai contului lor. În cazul în care înlocuitorii unui cont nu respectă aceste reguli și Termenii și Condițiile Generale ale Novaterra, atât proprietarul contului, cât și înlocuitorul pot fi considerați responsabili și pedepsiți.');
tz_def('TZ_THERE_IS_NO_COMPENSATION_FOR_DAMAG_2', 'Nu există compensație pentru daunele provocate de cineva care cunoaște parola unui cont. Persoana care primește parola este supusă regulilor Novaterra, precum și Termenilor și Condițiilor Generale.');
tz_def('TZ_THERE_IS_NO_SPECIAL_TREATMENT_FOR', 'Nu există un tratament special pentru utilizatorii Novaterra Plus/Aur în privința regulilor jocului, nici în timpul necesar pentru a trata cazul, nici în pedeapsă.');
...
tz_def('TZ_THE_FOLLOWING_SET_OF_RULES_ARE_IN', 'Următorul set de reguli este în legătură cu Termenii și Condițiile Generale ale Novaterra. Ar trebui să te familiarizezi cu Termenii și Condițiile Generale pentru a verifica ce este permis și ce este interzis, în special în cazul unui cont interzis pentru încălcarea regulilor.');
...
tz_def('TZ_THE_NOVATERRA_TEAM_RESERVES_THE_RIGH', 'Echipa Novaterra își rezervă dreptul de a modifica regulile în orice moment.');
...
tz_def('TZ_ML_CHANGELOG_120BUGS', 'Peste 120 de bug-uri reparate, artefacte complet reparate, catapulte și berbeci complet reparați, Tatari/Artefacte/sate MdL/planuri de construcție MdL automatizate, nouă formulă de luptă (mai precisă decât cea veche), activare automată a artefactelor, mult cod rescris. Mai multe în fișierul readme!');
tz_def('TZ_ML_CHANGELOG_NEWFORUM', 'Sistem nou de forum, formulă a capcanelor în stil Novaterra, maistru constructor reparat, coadă dublă de cercetare în fierărie și armurărie cu Plus');
...
tz_def('TZ_ML_GPACK_ALTER_APPEARANCE', 'Cu un pachet grafic poți schimba aspectul Novaterra. Poți alege unul din listă sau furniza o cale personalizată.');
...
tz_def('TZ_ML_ACCOUNT_FROZEN', '. În acest timp contul tău este practic înghețat. Nu vor progresa resurse, trupe sau cercetări, iar satele tale nu pot fi atacate. Amintește-ți, asta îți îngheață doar jocul, nu timpul.');
...
tz_def('MANUAL_NEW_FEATURES_DESC', "Acestea sunt funcții noi pe care nu le vei găsi în versiunea reală a jocului Novaterra T3.6. Aici te poți familiariza cu toate funcțiile noi în detaliu.");
tz_def('MANUAL_FAQ', 'Întrebări frecvente Novaterra');
...
tz_def('MANUAL_NPC_NATARS', "Această descriere are doar rol informativ. Tatarii sunt un trib pur NPC și, prin urmare, nu pot fi jucați.");
...
tz_def('MANUAL_UDESC_11', "Bâtașii sunt cea mai ieftină unitate din Novaterra. Sunt antrenați rapid și au capacități de atac medii, dar armura lor nu este cea mai bună. Bâtașii sunt aproape lipsiți de apărare în fața cavaleriei și sunt călcați în picioare cu ușurință.");
...
tz_def('MANUAL_UDESC_44', "Tatarii folosesc stoluri de păsări pentru a aduna informații despre dușmanii lor. Datorită avantajului cercetării din aer, este aproape imposibil să oprești escadroanele de cercetași tatarieni; pe de altă parte, chiar și un sătean simplu poate observa cu ușurință stolurile țipătoare și împănate.");
tz_def('MANUAL_UDESC_41', "Sulițele lor lungi și ascuțite sunt folosite ca principală linie de apărare în orice bătălie. Sulițașii tatarieni sunt războinici îndrăzneți și curajoși care își folosesc dexteritatea pentru a doborî rapid călăreții inamici și a-i răpune.");
tz_def('MANUAL_UDESC_42', "Prelungirile în formă de spini de pe coifurile, brățările și apărătorile de umăr ale armurii lor le dau Războinicilor cu spini numele. Bărbații care luptă pentru Tatari ca Războinici cu spini sunt perseverenți și bine antrenați, oferind o bătălie sângeroasă oricui este destul de nesăbuit să îi atace.");
tz_def('MANUAL_UDESC_43', "Adorați de poporul lor și temuți de dușmani, Gardienii luptă fără cal, dar sunt totuși printre cei mai valoroși soldați din armata tatariană, datorită versatilității lor. Sunt considerați luptători bine antrenați, lăsându-le inamicilor aproape nicio șansă de victorie. Datorită armurii lor grele, pot fi folosiți și ca trupe de apărare puternice și de încredere.");
...
tz_def('MANUAL_NF_T_21', "Manual despre Natură și Tatari");
...
tz_def('MANUAL_NF_D_11', "Dacă în sat există o oază capturată, aceasta va fi afișată în profilul jucătorului în dreptul satului corespunzător, cu tipul de resursă corespunzător și un bonus de producție. Această funcție a fost prezentată în Novaterra T4.");
...
tz_def('MANUAL_NF_D_17', "Dacă într-unul dintre sate există un artefact, acesta va fi afișat în profilul jucătorului în dreptul satului corespunzător în care se află. Această funcție a fost prezentată în Novaterra T4.");
tz_def('MANUAL_NF_D_18', "Dacă în sat există un loc pentru construirea Minunii Lumii, acesta va fi afișat în profil în dreptul satului corespunzător. Această funcție a fost prezentată în Novaterra T4.");
tz_def('MANUAL_NF_D_19', "Modul vacanță îți permite să îți protejezi imperiul de orice acțiune ostilă a altor jucători în timpul absenței tale îndelungate. Este adevărat, există anumite condiții care vor duce la o întârziere în dezvoltarea imperiului tău. Această funcție a fost prezentată în Novaterra T4.");
tz_def('MANUAL_NF_D_20', "Dacă trimiți catapultele într-un atac normal, poți vedea în punctul de adunare ce ținte ai stabilit pentru atac. Această funcție a fost prezentată în Novaterra T4.");
tz_def('MANUAL_NF_D_21', "Cu ajutorul acestor informații din manual, poți găsi o descriere a forțelor Naturii și ale Tatarilor.");
tz_def('MANUAL_NF_D_22', "Locația linkurilor directe se schimbă. În Novaterra T3.6 original, linkurile directe sunt plasate în meniul din dreapta, sub lista satelor, ceea ce nu este pe deplin convenabil. Dacă această opțiune este activată, linkurile directe vor fi plasate în meniul din stânga, ceea ce este mult mai convenabil.");
tz_def('MANUAL_NF_D_23', "Medalie acordată jucătorilor care folosesc aceeași adresă de e-mail de 3 ani sau mai mult. Poate fi adăugată la descrierea profilului. Această funcție a fost prezentată în Novaterra T4.");
tz_def('MANUAL_NF_D_24', "Medalie acordată jucătorilor care folosesc aceeași adresă de e-mail de 5 ani sau mai mult. Poate fi adăugată la descrierea profilului. Această funcție a fost prezentată în Novaterra T4.");
tz_def('MANUAL_NF_D_25', "Medalie acordată jucătorilor care folosesc aceeași adresă de e-mail de 10 ani sau mai mult. Poate fi adăugată la descrierea profilului. Această funcție a fost prezentată în Novaterra T4.");
tz_def('MANUAL_NF_D_26', "Medalie acordată jucătorilor care folosesc aceeași adresă de e-mail de 10 ani sau mai mult. Poate fi adăugată la descrierea profilului. Această funcție a fost prezentată în Novaterra T4.");
...
tz_def('HB_TXT_VS_NATARS',  'forta impotriva Tatarilor');
...
tz_def('ADMIN_NATARS_MANAGEMENT', 'Administrare Tatari');
...
tz_def('ADM_MANUAL_ON_NATURE_AND_NATARS', 'Manual despre Natura si Tatari');
tz_def('ADM_ENABLE_DISABLE_MANUAL_INFO', 'Activeaza (dezactiveaza) informatiile din manual');
tz_def('ADM_ENABLE_DISABLE_DISPLAYING_INFORMATION_IN_THE', 'Activeaza (dezactiveaza) afisarea in Manual a informatiilor despre trupele Naturii si ale Tatarilor');
...
tz_def('ADM_IF_ENABLED_THEN_THE_DIRECT_LINKS_WILL_BE_PLA', 'Daca e activat, linkurile directe apar in meniul din stanga; daca e dezactivat, in meniul din dreapta, ca in Novaterra-ul original');
...
tz_def('ADM_YES', 'Da');
tz_def('ADM_NO', 'Nu');
tz_def('ADM_NOVATERRA_OFFICIAL', 'Novaterra oficial');
tz_def('ADM_NOVATERRA_EXTENDED', 'Novaterra extins');
...
tz_def('ADM_ACTIVATE_NOVATERRA_PLUS_FOR_ALL_PLAYERS_ON_THE', 'Activeaza Novaterra Plus pentru TOTI jucatorii de pe server.');
...
tz_def('ADM_RETURN_TO_NATARS', 'Returneaza Tatarilor');
...
tz_def('ADM_MAD_ON', 'PORNIT');
tz_def('ADM_MAD_OFF', 'OPRIT');
tz_def('ADM_MAD_AUTO_BAN', 'Blocare Automata');
tz_def('ADM_MAD_ENABLE_DETECTION', 'Detectare Multi-Cont');
tz_def('ADM_MAD_ENABLE_AUTO_BAN', 'Blocare Automata');
tz_def('ADM_MAD_AUTO_BAN_THRESHOLD', 'Prag scor pentru blocare automata');
tz_def('ADM_MAD_AUTO_BAN_HINT', 'Cand ambele sunt pornite, orice pereche care atinge pragul este blocata automat imediat ce aceasta pagina se incarca sau aceste setari sunt salvate.');
tz_def('ADM_MAD_AUTO_BAN_JUST_RAN', 'Blocarea automata tocmai a rulat');
tz_def('ADM_MAD_FULL_ADMIN_ONLY', 'Doar un administrator complet poate schimba aceste setari.');
tz_def('ADM_MAD_DETECTION_DISABLED_NOTICE', 'Detectarea Multi-Cont este momentan oprita. Porneste-o mai sus pentru a cauta perechi de conturi suspecte.');
tz_def('ADM_MAD_VIEW_PAIR', 'Vezi');
tz_def('ADM_MAD_PLAYER_DASHBOARD', 'Administrare Multi-Cont');
tz_def('ADM_MAD_BACK_TO_OVERVIEW', 'Inapoi la Detectarea Multi-Cont');
tz_def('ADM_MAD_BANNED', 'Banat');
tz_def('ADM_MAD_ACTIVE', 'Activ');
tz_def('ADM_MAD_RELATED_PROTECTION_OFF_NOTICE', 'Protectia Conturilor Asociate este momentan oprita — jefuirea intre conturi asociate nu este blocata.');
tz_def('ADM_MAD_ACCOUNT_ACTIONS', 'Actiuni');
tz_def('ADM_MAD_BAN_ACCOUNT', 'Baneaza Contul');
tz_def('ADM_MAD_UNBAN_ACCOUNT', 'Debaneaza Contul');
tz_def('ADM_MAD_BAN_REASON', 'Motiv');
tz_def('ADM_MAD_RELATE_TO_UID', 'UID cont asociat');
tz_def('ADM_MAD_REASON_OPTIONAL', 'Motiv (optional)');
tz_def('ADM_MAD_MARK_AS_RELATED', 'Marcheaza ca Asociat');
tz_def('ADM_MAD_SUSPECTED_PAIRS', 'Perechi Suspecte Detectate Automat');
tz_def('ADM_MAD_NO_SUSPECTED_PAIRS', 'Nu au fost detectate perechi suspecte pentru acest cont.');
tz_def('ADM_MAD_OTHER_ACCOUNT', 'Cont');
tz_def('ADM_MAD_RELATED_ACCOUNTS', 'Conturi Asociate Manual');
tz_def('ADM_MAD_NO_RELATED_ACCOUNTS', 'Niciun cont asociat manual cu acesta.');
tz_def('ADM_MAD_REASON', 'Motiv');
tz_def('ADM_MAD_ADDED', 'Adaugat');
tz_def('ADM_MAD_REMOVE', 'Elimina');
tz_def('ADM_MAD_IPS_AND_DEVICES', 'IP-uri si Dispozitive');
tz_def('ADM_MAD_NO_FINGERPRINTS', 'Nicio amprenta IP/dispozitiv inregistrata inca.');
tz_def('ADM_MAD_HITS', 'Aparitii');
tz_def('ADM_MAD_LAST_SEEN', 'Vazut ultima data');
tz_def('ADM_MAD_BLOCKED_ATTEMPTS', 'Incercari de Jefuire/Transfer Blocate');
tz_def('ADM_MAD_NO_BLOCKED_ATTEMPTS', 'Nicio incercare blocata inregistrata pentru acest cont.');
tz_def('ADM_MAD_FROM', 'De la');
tz_def('ADM_MAD_TO', 'Catre');
tz_def('ADM_MAD_RESOURCES', 'Resurse');
tz_def('ADM_MAD_WHEN', 'Cand');
tz_def('ADM_MAD_RECENT_LOGINS', 'Autentificari Recente');
tz_def('ADM_MAD_NO_LOGIN_RECORDS', 'Niciun istoric de autentificare gasit.');
tz_def('ADM_MAD_BAN_HISTORY', 'Istoric Banari');
tz_def('ADM_MAD_NO_BAN_HISTORY', 'Niciun istoric de banare pentru acest cont.');
tz_def('ADM_MAD_STATUS', 'Stare');
tz_def('ADM_MAD_LIFTED', 'Ridicat');
...
tz_def('ADM_TO', 'catre');
tz_def('ADM_RECIPIENT', 'Destitatar');
...
tz_def('ADM_SECURE_ACCESS_TO_NOVATERRA_PANEL', 'Acces securizat la panoul Novaterra');
...
tz_def('ADM_THIS_ACTION_WILL_DISABLE_NOVATERRA_PLUS_FOR_AL', 'Aceasta actiune va dezactiva Novaterra Plus pentru TOTI jucatorii.');
...
tz_def('ADM_USE_IT_TO_PRIORITISE_WHICH_PAIRS_A_HUMAN_SHO', '&mdash; foloseste-l ca sa stabilesti ce perechi merita investigate de un om. Nimic nu se blocheaza automat, decat daca administratorul activeaza Blocarea Automata mai jos.');
...
tz_def('PLUSSTATS_NEEDPLUS', 'Statisticile grafice sunt o functie Novaterra Plus. Activeaza Plus ca sa vezi evolutia contului.');
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
tz_def('TZ_WW_NATARS', 'Tatari');
tz_def('CONF_SERV_NATARS_WW_START_DELAY', 'Pornirea Minunii Tatarilor');
tz_def('CONF_SERV_NATARS_WW_START_DELAY_TOOLTIP', 'Dupa cate zile de la aparitia planurilor incep Tatarii propria Minune a Lumii. Se scaleaza cu viteza serverului. 0 dezactiveaza.');
...
tz_def('TZ_WS_VS_NATARS', 'Contra Tatarilor');
