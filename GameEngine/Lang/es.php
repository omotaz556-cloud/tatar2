...
tz_def('NATAR_COUNTERFORCE', 'Contraataque de los Tatares');
...
tz_def('ONE_DAY_OF_NOVATERRA', '1 día de Novaterra ');
tz_def('TWO_DAYS_OF_NOVATERRA', '2 días de Novaterra ');
...
tz_def('U46', 'Caballero tatario');
tz_def('U47', 'Elefante de guerra');
tz_def('U48', 'Balista');
tz_def('U49', 'Emperador tatario');
tz_def('U50', 'Colono tatario');
//HUNS (TRIBE 6)
...
tz_def('ADM_MAD_ON', 'ACTIVADO');
tz_def('ADM_MAD_OFF', 'DESACTIVADO');
tz_def('ADM_MAD_AUTO_BAN', 'Ban Automatico');
tz_def('ADM_MAD_ENABLE_DETECTION', 'Deteccion Multi-Cuenta');
tz_def('ADM_MAD_ENABLE_AUTO_BAN', 'Ban Automatico');
tz_def('ADM_MAD_AUTO_BAN_THRESHOLD', 'Umbral de puntuacion para ban automatico');
tz_def('ADM_MAD_AUTO_BAN_HINT', 'Cuando ambos estan activados, cualquier par que alcance el umbral se banea automaticamente al cargar esta pagina o al guardar esta configuracion.');
tz_def('ADM_MAD_AUTO_BAN_JUST_RAN', 'El ban automatico se acaba de ejecutar');
tz_def('ADM_MAD_FULL_ADMIN_ONLY', 'Solo un administrador completo puede cambiar esta configuracion.');
tz_def('ADM_MAD_DETECTION_DISABLED_NOTICE', 'La Deteccion Multi-Cuenta esta desactivada actualmente. Activala arriba para buscar pares de cuentas sospechosas.');
tz_def('ADM_MAD_VIEW_PAIR', 'Ver');
tz_def('ADM_MAD_PLAYER_DASHBOARD', 'Administracion Multi-Cuenta');
tz_def('ADM_MAD_BACK_TO_OVERVIEW', 'Volver a Deteccion Multi-Cuenta');
tz_def('ADM_MAD_BANNED', 'Baneado');
tz_def('ADM_MAD_ACTIVE', 'Activo');
tz_def('ADM_MAD_RELATED_PROTECTION_OFF_NOTICE', 'La Proteccion de Cuentas Relacionadas esta desactivada actualmente — no se bloquea el saqueo entre cuentas relacionadas.');
tz_def('ADM_MAD_ACCOUNT_ACTIONS', 'Acciones');
tz_def('ADM_MAD_BAN_ACCOUNT', 'Banear Cuenta');
tz_def('ADM_MAD_UNBAN_ACCOUNT', 'Desbanear Cuenta');
tz_def('ADM_MAD_BAN_REASON', 'Motivo');
tz_def('ADM_MAD_RELATE_TO_UID', 'UID de cuenta relacionada');
tz_def('ADM_MAD_REASON_OPTIONAL', 'Motivo (opcional)');
tz_def('ADM_MAD_MARK_AS_RELATED', 'Marcar como Relacionada');
tz_def('ADM_MAD_SUSPECTED_PAIRS', 'Pares Sospechosos Detectados');
tz_def('ADM_MAD_NO_SUSPECTED_PAIRS', 'No se detectaron pares sospechosos para esta cuenta.');
tz_def('ADM_MAD_OTHER_ACCOUNT', 'Cuenta');
tz_def('ADM_MAD_RELATED_ACCOUNTS', 'Cuentas Relacionadas Manualmente');
tz_def('ADM_MAD_NO_RELATED_ACCOUNTS', 'No hay cuentas relacionadas manualmente con esta.');
tz_def('ADM_MAD_REASON', 'Motivo');
tz_def('ADM_MAD_ADDED', 'Agregado');
tz_def('ADM_MAD_REMOVE', 'Quitar');
tz_def('ADM_MAD_IPS_AND_DEVICES', 'IPs y Dispositivos');
tz_def('ADM_MAD_NO_FINGERPRINTS', 'No hay huellas de IP/dispositivo registradas aun.');
tz_def('ADM_MAD_HITS', 'Veces');
tz_def('ADM_MAD_LAST_SEEN', 'Visto por ultima vez');
tz_def('ADM_MAD_BLOCKED_ATTEMPTS', 'Intentos de Saqueo/Transferencia Bloqueados');
tz_def('ADM_MAD_NO_BLOCKED_ATTEMPTS', 'No hay intentos bloqueados registrados para esta cuenta.');
tz_def('ADM_MAD_FROM', 'De');
tz_def('ADM_MAD_TO', 'Para');
tz_def('ADM_MAD_RESOURCES', 'Recursos');
tz_def('ADM_MAD_WHEN', 'Cuando');
tz_def('ADM_MAD_RECENT_LOGINS', 'Inicios de Sesion Recientes');
tz_def('ADM_MAD_NO_LOGIN_RECORDS', 'No se encontraron registros de inicio de sesion.');
tz_def('ADM_MAD_BAN_HISTORY', 'Historial de Baneos');
tz_def('ADM_MAD_NO_BAN_HISTORY', 'No hay historial de baneos para esta cuenta.');
tz_def('ADM_MAD_STATUS', 'Estado');
tz_def('ADM_MAD_LIFTED', 'Levantado');
...
tz_def('ADM_USE_IT_TO_PRIORITISE_WHICH_PAIRS_A_HUMAN_SHO', '&mdash; usalo para priorizar que pares debe investigar un humano. Nada se banea automaticamente a menos que el administrador active el Ban Automatico abajo.');
...
tz_def('PLUSSTATS_NEEDPLUS', 'Las estadísticas gráficas son una función de Novaterra Plus. Activa Plus para ver el desarrollo de tu cuenta.');
...
tz_def('TZ_WS_VS_NATARS', 'Contra Tatares');
