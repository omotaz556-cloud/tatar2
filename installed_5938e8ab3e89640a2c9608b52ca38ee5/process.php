...
class Process {
	...
	private function constForm() {
		...
		// contine novaterra.css, altfel jocul ar porni fara stiluri.
		$gpLocate = isset($_POST['gp_locate']) ? (string) $_POST['gp_locate'] : 'gpack/novaterra_classic/';

		if (!preg_match('#^gpack/[A-Za-z0-9_\-]+/$#', $gpLocate)
			|| !is_file('../' . $gpLocate . 'novaterra.css')) {
			$gpLocate = 'gpack/novaterra_classic/';
		}
