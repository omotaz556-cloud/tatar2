<?php
/**
 * Greek.sa literal alliance ranking table — matches reference screenshot.
 */
...
?>
<table class="b0 fl L6_S gk-sta-ally">
<colgroup>
...
<span class="staTit"><?php echo TZ_THE_LARGEST_ALLIANCES; ?></span>
<span class="staLe" aria-hidden="true"></span>
</div></th></tr>
<tr class="gk-sta-cols"><th class="ra">#</th><th class="al"><?php echo ALLIANCE; ?></th><th class="pla"><?php echo PLAYERS; ?></th><th class="po"><?php echo defined('TZ_POINTS') ? TZ_POINTS : 'النقاط'; ?></th></tr>
<?php
if (count($rankArray) > 1) {
    for ($i = $start; $i < $start + 20; $i++) {
        ...
        echo '<a href="allianz.php?aid=' . $aid . '">'
            . htmlspecialchars((string) $row['tag'], ENT_QUOTES, 'UTF-8') . '</a>';
        echo '</th>';
        echo '<th class="pla">' . (int) ($row['players'] ?? 0) . '</th>';
        echo '<th class="po' . $lcCls . '"><bdi dir="ltr">' . number_format((int) ($row['totalpop'] ?? 0)) . '</bdi></th>';
        echo '</tr>';
    }
} else {
