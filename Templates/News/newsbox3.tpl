<?php
/**
 * ==========================================================
 * Novaterra Newsbox3 - SAFE REFACTOR
 * ==========================================================
 * - fără modificări de logică
 * - curățare HTML
 * - structură mai lizibilă
 * ==========================================================
 */
?>

<h5>
    <img src="../img/en/t2/newsbox3.gif" alt="<?php echo EDIT_NEWSBOX3; ?>" />
</h5>

<div class="news">

    <div style="text-align: center;">
        <b><?php echo TZ_NOVATERRA; ?></b>
    </div>

    <br />

    <b><?php echo TZ_VERSION; ?> <font color="Red">v8.3.5</font></b><br />

    <b><?php echo TZ_MAJOR_CHANGES; ?></b>
    <?php echo TZ_ML_CHANGELOG_120BUGS; ?>
    <br />

    <b><?php echo TZ_MINOR_CHANGES; ?></b>
    <?php echo TZ_ML_CHANGELOG_NEWFORUM; ?>
    <br />

    <b><?php echo TZ_SCRIPT_PRICE; ?></b> <?php echo TZ_FREE; ?>
    <br /><br />

    <div style="text-align: center;">
        <u><b><?php echo TZ_RELEASED_BY_NOVATERRA_TEAM; ?></b></u>
    </div>

    <br />

    <div style="text-align: center;">
        <b>
            <?php echo TZ_VISIT; ?>
            <a href="http://forum.ragezone.com/f583/novaterra-version-modified-shadow-many-967580/" target="_blank">
                <?php echo TZ_RAGEZONE_COM; ?>
            </a>
        </b>
    </div>

    <br />

    <div style="text-align: center;">
        <b>
            <u>
                <?php echo TZ_DOWNLOAD_FROM; ?>
                <a href="https://github.com/omotaz556-cloud/tatar/archive/master.zip" target="_blank">
                    <?php echo TZ_GITHUB; ?>
                </a>
            </u>
        </b>
    </div>

    <br />

</div>