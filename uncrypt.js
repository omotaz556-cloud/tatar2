...
window.Novaterra = {
			...
			if (Browser.Engine.webkit) {
				...
				if (Browser.Engine.presto) {
					...
					if (Browser.Engine.trident && window.console) {
						...
						if (!$("novaterra_console")) {
							var a = new Element("div", {
								id: "novaterra_console",
								styles: {
									position: "absolute",
									left: 0,
									height: 150,
							...
							html: b + "<br />"
						})).inject($("novaterra_console"), "bottom")
					}
				}
			}
		}
	...
	ajax: function (a) {
		a = a || {};
		var b = {
			onRequest: a.onRequest || Novaterra.emptyFunction,
			onComplete: a.onComplete || Novaterra.emptyFunction,
			onCancel: a.onCancel || Novaterra.emptyFunction,
			onSuccess: a.onSuccess || Novaterra.emptyFunction,
			onFailure: a.onFailure || Novaterra.emptyFunction,
			onException: a.onException || Novaterra.emptyFunction
		};
		if (!a.url) {
			a.url = "ajax.php"
		}
...
Novaterra.ajax = Novaterra.ajax.wrap(function (b, a) {
	if (!a.url) {
		a.url = "ajax.php"
	}
	return b(a)
...
Novaterra.Main = {};
Novaterra.Main.Flags = new Class({
	Implements: [Options],
	currentRegion: null,
	elements: {
		container: null,
